...
from octopusos.core.answers import (
    AnswerStore,
    AnswerValidator,
...
@click.option("--ui", type=click.Choice(["cli", "tui"]), default="cli",
              help="UI mode for AnswerPack creation")
...
def create_cmd(
    ...
    try:
        if ui == "tui":
            if not interactive:
                console.print("[yellow]Warning: --no-interactive ignored for TUI mode[/yellow]")
            from octopusos.ui.answer_tui import run_answer_tui
...
def apply_cmd(intent_path: str, answer_pack_path: str, output_path: str):
    ...
    try:
        ...
        # Apply
        from octopusos.core.answers import AnswerApplier
