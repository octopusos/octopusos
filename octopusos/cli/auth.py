...
def add_profile(
    ...
    Examples:
        ...
        # Add SSH key profile
        octopusos auth add --name work-ssh --type ssh_key --key-path ~/.ssh/id_rsa

        # Add GitHub PAT profile
        octopusos auth add --name github-personal --type pat_token --token <token> --provider github

        # Add netrc profile
        octopusos auth add --name gitlab-netrc --type netrc --machine gitlab.com --login user --password <pass>
    """
    ...
    try:
        ...
        if click.confirm("Validate credentials now?", default=True):
            ...
            if profile_type == AuthProfileType.PAT_TOKEN:
                ...
                click.echo("   Test with: octopusos auth validate <profile-name> --url <git-url>")
...
def list_profiles(verbose: bool):
    ...
    try:
        ...
        if not profiles:
            ...
            click.echo("\nAdd one with: octopusos auth add --help")
            return
...
def remove_profile(profile_name: str, yes: bool):
    ...
    Example:
        octopusos auth remove github-personal
    """
    manager = CredentialsManager()
...
def validate_profile(profile_name: str, url: Optional[str]):
    ...
    Examples:
        ...
        octopusos auth validate github-personal

        # Validate SSH key with custom URL
        octopusos auth validate work-ssh --url git@github.com:org/repo.git
    """
    manager = CredentialsManager()
    git_client = GitClientWithAuth(manager)
...
def show_profile(profile_name: str):
    ...
    Example:
        octopusos auth show github-personal
    """
    manager = CredentialsManager()
