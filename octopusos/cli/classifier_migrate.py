...
- octopusos classifier migrate --shadow v2-shadow-a --to-active
- octopusos classifier migrate --dry-run --shadow v2-shadow-a
- octopusos classifier migrate --verify --shadow v2-shadow-a
- octopusos classifier migrate --rollback
...
from octopusos.config.loader import load_shadow_config, save_shadow_config
from octopusos.core.brain.classifier_version_manager import get_version_manager
from octopusos.core.brain.improvement_proposal_store import get_store
from octopusos.core.chat.shadow_registry import get_shadow_registry
from octopusos.core.chat.decision_comparator import DecisionComparator
        ...
        if not skip_verification:
            ...
            if not prereqs.is_valid():
                raise ValueError(
                    ...
                    f"Run 'octopusos classifier migrate --verify --shadow {shadow_version}' "
                    f"for details."
                )
        else:
...
def migrate_to_active_cmd(
    ...
    Examples:
        ...
        octopusos classifier migrate to-active --shadow v2-shadow-a

        # Dry run to preview changes
        octopusos classifier migrate to-active --shadow v2-shadow-a --dry-run

        # Skip verification (not recommended)
        octopusos classifier migrate to-active --shadow v2-shadow-a --skip-verification
    """
    try:
        ...
        if dry_run:
            ...
            console.print()
            console.print("[yellow]Note:[/yellow] Use 'octopusos classifier migrate rollback' if issues arise")
...
def rollback_migration_cmd(reason: str, user: str, yes: bool):
    ...
    Example:
        octopusos classifier migrate rollback --reason "Performance regression detected"
    """
    try:
        manager = ClassifierMigrationManager()
