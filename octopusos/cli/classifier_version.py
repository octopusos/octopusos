...
- octopusos version promote --proposal BP-017 [--major]
- octopusos version rollback --to v1 --reason "..."
- octopusos version list
- octopusos version show v2
- octopusos version history
"""
...
def run_async(coro: Awaitable[T]) -> T:
    ...
    try:
        ...
        # Event loop already running; run coroutine in a separate thread/loop.
        result: T
...
def promote_version_cmd(proposal: str, major: bool, user: str):
    ...
    Examples:
        ...
        octopusos version promote --proposal BP-017

        # Major version bump (v2 -> v3)
        octopusos version promote --proposal BP-017 --major

        # Specify user
        octopusos version promote --proposal BP-017 --user alice
    """
    try:
...
def rollback_version_cmd(to_version: str, reason: str, user: str, yes: bool):
    ...
    Examples:
        # Rollback to v2
        octopusos version rollback --to v2 --reason "Performance regression"

        # Skip confirmation
        octopusos version rollback --to v2 --reason "Bug detected" --yes
    """
    try:
        manager = get_version_manager()
...
def list_versions_cmd(limit: int):
    ...
    Example:
        octopusos version list
        octopusos version list --limit 50
    """
    try:
        manager = get_version_manager()
...
def rollback_history_cmd(limit: int):
    ...
    Example:
        octopusos version history
        octopusos version history --limit 50
    """
    try:
        manager = get_version_manager()
