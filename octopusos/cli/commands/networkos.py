...
def create(provider, name, hostname, target, token, mode):
    ...
    try:
        ...
        click.echo(f"\nStart with: octopusos networkos start {tunnel_id}")
    ...
    try:
        ...
        if events:
            ...
            for event in events:
                ...
                from octopusos.core.time import from_epoch_ms
...
def migrate_secrets(tunnel_id, check):
    ...
    Security Benefits:
    - Tokens stored encrypted in ~/.octopusos/secrets.json (0600 permissions)
    ...
    Examples:
        ...
        octopusos networkos migrate-secrets --check

        # Migrate all tunnels
        octopusos networkos migrate-secrets

        # Migrate specific tunnel
        octopusos networkos migrate-secrets TUNNEL_ID
    """
    try:
        service = NetworkOSService()
        ...
        if count > 0:
            ...
            click.echo(f"  • Tokens now encrypted in ~/.octopusos/secrets.json")
        ...
        click.echo("2. Check ~/.octopusos/secrets.json permissions (should be 0600)", err=True)
