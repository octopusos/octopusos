...
Commands:
- octopusos skill list: List all skills
- octopusos skill info <skill_id>: Show skill details
- octopusos skill import <source>: Import skill from local path or GitHub
- octopusos skill enable <skill_id>: Enable skill (requires admin token)
- octopusos skill disable <skill_id>: Disable skill (requires admin token)
"""
...
from octopusos.skills.importer.local_importer import LocalImporter
from octopusos.skills.importer.github_importer import GitHubImporter, GitHubFetchError
...
def import_(source):
    ...
    Examples:
        ...
        octopusos skill import github:owner/repo

        octopusos skill import github:owner/repo#main

        octopusos skill import github:owner/repo#v1.0.0:skills/example
    """
    registry = SkillRegistry()
    ...
    try:
        if source.startswith('github:'):
            ...
            click.echo(f"   octopusos skill enable {skill_id} --token <your-admin-token>")
...
@skill.command()
@click.argument('skill_id')
@click.option('--token', envvar='OCTOPUSOS_ADMIN_TOKEN', required=False,
              help='Admin token (or set OCTOPUSOS_ADMIN_TOKEN environment variable)')
def enable(skill_id, token):
    """Enable a skill (requires admin token)."""
    if not token:
        click.echo("❌ Admin token required. Provide via --token or OCTOPUSOS_ADMIN_TOKEN env var.", err=True)
        click.echo("\n💡 Example:")
        click.echo("   export OCTOPUSOS_ADMIN_TOKEN='your-token-here'")
        click.echo("   octopusos skill enable my-skill")
        return
