...
Usage:
    octopusos task dependencies <task_id>                # Show dependencies
    octopusos task dependencies <task_id> --reverse      # Show reverse dependencies
    octopusos task dependencies --graph                  # Export full DAG
    octopusos task dependencies --graph --output file.dot # Export to file
    octopusos task dependencies --check-cycles           # Check for circular dependencies
"""
...
from octopusos.core.task.models import DependencyType
...
def show_dependencies(task_id: str, reverse: bool, format: str):
    ...
    Examples:
        octopusos task dependencies show task-123
        octopusos task dependencies show task-123 --reverse
        octopusos task dependencies show task-123 --format json
    """
    try:
        db = get_db()
...
def export_graph(output: str, format: str, tasks: tuple):
    ...
    Examples:
        octopusos task dependencies graph
        octopusos task dependencies graph -o deps.dot
        octopusos task dependencies graph --format json -o deps.json
        octopusos task dependencies graph --tasks task-001 --tasks task-002
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
def check_cycles():
    ...
    Examples:
        octopusos task dependencies check-cycles
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
def show_ancestors(task_id: str, format: str):
    ...
    Examples:
        octopusos task dependencies ancestors task-123
        octopusos task dependencies ancestors task-123 --format json
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
def show_descendants(task_id: str, format: str):
    ...
    Examples:
        octopusos task dependencies descendants task-123
        octopusos task dependencies descendants task-123 --format json
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
def topological_sort(format: str):
    ...
    Examples:
        octopusos task dependencies topological-sort
        octopusos task dependencies topological-sort --format json
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
def create_dependency(task_id: str, depends_on_task_id: str, dep_type: str, reason: str, safe: bool):
    ...
    Examples:
        octopusos task dependencies create task-002 task-001 --type requires --reason "Needs output"
        octopusos task dependencies create task-003 task-002 --type blocks --reason "Must wait" --safe
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
def delete_dependency(task_id: str, depends_on_task_id: str, dep_type: str):
    ...
    Examples:
        octopusos task dependencies delete task-002 task-001
        octopusos task dependencies delete task-002 task-001 --type requires
    """
    try:
        db = get_db()
        service = TaskDependencyService(db)
...
from octopusos.cli.commands.task_trace import task_trace
