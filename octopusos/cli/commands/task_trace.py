...
This module provides the `octopusos task trace` command to view:
...
Usage:
    octopusos task trace <task_id> [--format table|json|tree] [--detailed]
    octopusos task trace task-123
    octopusos task trace task-123 --format json
    octopusos task trace task-123 --detailed
...
from octopusos.core.task.artifact_service import TaskArtifactService, ArtifactRefType
...
def task_trace(task_id: str, output_format: str, detailed: bool):
    ...
    Examples:
        ...
        octopusos task trace task-123 --format json
        octopusos task trace task-123 --detailed
        octopusos task trace task-123 --format tree
    """
    try:
        db = get_db()
        db_path = get_db_path()
        ...
        for scope_data in repo_scopes:
            ...
            if repo_row:
                ...
                elif hasattr(repo_row, "keys"):
                    if "name" in repo_row.keys():
                        repo_name = repo_row["name"]
