...
from octopusos.core.task.spec_service import TaskSpecService
from octopusos.core.task.binding_service import BindingService
from octopusos.core.project.service import ProjectService
from octopusos.core.project.repo_service import RepoService
...
def create_task(
    ...
    try:
        ...
        console.print("\n[dim]Next steps:[/dim]")
        console.print(f"  1. Review spec: [cyan]octopusos task-v31 show {task.task_id}[/cyan]")
        console.print(f"  2. Freeze spec: [cyan]octopusos task-v31 freeze {task.task_id}[/cyan]")
        if not repo_id:
            console.print(f"  3. Bind to repo: [cyan]octopusos task-v31 bind {task.task_id} --project {project_id} --repo <repo_id>[/cyan]")
        console.print(f"  4. Mark ready: [cyan]octopusos task-v31 ready {task.task_id}[/cyan]")
...
def freeze_task_spec(task_id: str, acceptance_criteria: tuple, output_json: bool):
    ...
    try:
        ...
        console.print("\n[dim]Next step:[/dim]")
        console.print(f"  Mark as ready: [cyan]octopusos task-v31 ready {task_id}[/cyan]")
