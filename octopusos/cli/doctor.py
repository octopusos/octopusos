"""
octopusos doctor - Environment health checker and auto-fixer

Usage:
    octopusos doctor              # Check only (read-only)
    octopusos doctor --fix        # Auto-fix issues
    octopusos doctor --fix --python 3.13  # Specify Python version (optional)
...
from octopusos.core.doctor import (
    run_all_checks,
    apply_all_fixes,
    print_report,
    print_fix_summary,
...
def doctor(fix: bool, python: str):
    ...
    示例：
        ...
        octopusos doctor              # 检查环境
        octopusos doctor --fix        # 一键修复
        octopusos doctor --fix --python 3.13  # 指定 Python 版本
    """
    # Get project root (assume we're in octopusos/cli, go up 2 levels)
    try:
    ...
    # Print WebUI daemon diagnostics from the shared runtime source
    status = read_status()
    runtime_paths = get_runtime_paths()
    console.print("\n[bold cyan]WebUI Daemon[/bold cyan]")
    console.print(f"  Running: {'yes' if status.running else 'no'}")
    console.print(f"  URL: {status.url}")
    console.print(f"  PID: {status.pid if status.pid else '-'}")
    console.print(f"  Data dir: {status.data_dir}")
    console.print(f"  Log file: {status.log_file}")
    console.print(f"  Control token file: {runtime_paths.token_file}")
    console.print(f"  Last error: {status.last_error or '-'}")
