...
def task_inspect(task_id: str, db: Optional[str], format: str):
    ...
    Example:
        octopusos task inspect task_abc123
        octopusos task inspect task_abc123 --format json
    """
...
def governance_trace(task_id: str, db: Optional[str], limit: int, format: str):
    ...
    Example:
        octopusos governance trace task_abc123
        octopusos governance trace task_abc123 --limit 50 --format json
    """
    db_path = _resolve_db_path(db)
    if not db_path.exists():
        click.echo(f"❌ Database not found: {db_path}", err=True)
