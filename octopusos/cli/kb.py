...
Provides commands:
- octopusos kb search <query>  - Search documents
- octopusos kb refresh         - Refresh index
- octopusos kb explain <chunk_id> - Explain results
- octopusos kb stats           - Show statistics
"""
...
def search(query, scope, doc_type, top_k, explain, output_json, rerank):
    ...
    Examples:
        octopusos kb search "JWT authentication"
        octopusos kb search "API design" --scope docs/architecture/
        octopusos kb search "deployment" --doc-type runbook
        octopusos kb search "authentication" --rerank  # Force use rerank
    """
    try:
...
def refresh(changed_only, verbose):
    ...
    Examples:
        octopusos kb refresh              # Incremental refresh
        octopusos kb refresh --full       # Full refresh
        octopusos kb refresh --verbose    # Show detailed information
    """
    try:
        kb_service = ProjectKBService()
...
def explain(chunk_id):
    ...
    Examples:
        octopusos kb explain chunk_abc123
    """
    try:
        kb_service = ProjectKBService()
...
def inspect(chunk_id):
    ...
    Examples:
        octopusos kb inspect chunk_abc123
    """
    try:
        from octopusos.core.command import get_registry, CommandContext
...
def eval(queries_file, k_values, rerank, output_json):
    ...
    Examples:
        octopusos kb eval queries.jsonl
        octopusos kb eval queries.jsonl --k-values 1,5,10
        octopusos kb eval queries.jsonl --rerank
    """
    try:
        from octopusos.core.command import get_registry, CommandContext
...
def reindex(confirm):
    ...
    Examples:
        octopusos kb reindex --confirm
    """
    if not confirm:
        console.print("[red]Error:[/red] This is a dangerous operation.")
        console.print("Use --confirm to proceed: octopusos kb reindex --confirm")
        return
...
def repair(rebuild_fts, allow_drift, cleanup_orphans, explain):
    ...
    Examples:
        octopusos kb repair                     # 基础检查
        octopusos kb repair --rebuild-fts       # 重建 FTS（强一致性）
        octopusos kb repair --rebuild-fts --allow-drift  # 重建（容忍并发差异）
    """
    try:
...
def build(batch_size, verbose):
    ...
    Install first: pip install octopusos[vector]

    Examples:
        octopusos kb embed build
        octopusos kb embed build --batch-size 64
    """
    try:
        kb_service = ProjectKBService()
        ...
        if not kb_service.embedding_manager:
            ...
            console.print("Install deps: pip install octopusos[vector]")
            return
