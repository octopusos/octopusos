...
from octopusos.core.maintenance import DatabaseCleaner
...
def cleanup_orphans(days: int, dry_run: bool):
    ...
    Examples:
        ...
        octopusos maintenance cleanup-orphans --dry-run

        # Delete orphan tasks older than 30 days
        octopusos maintenance cleanup-orphans

        # Delete orphan tasks older than 60 days
        octopusos maintenance cleanup-orphans --days 60
    """
...
def cleanup_sessions(days: int, dry_run: bool):
    ...
    Examples:
        # Dry run
        octopusos maintenance cleanup-sessions --dry-run

        # Delete sessions inactive for 90+ days
        octopusos maintenance cleanup-sessions

        # Delete sessions inactive for 180+ days
        octopusos maintenance cleanup-sessions --days 180
    """
    cleaner = DatabaseCleaner()
...
def full_cleanup(orphan_days: int, session_days: int, vacuum: bool, dry_run: bool):
    ...
    Examples:
        # Dry run
        octopusos maintenance full-cleanup --dry-run

        # Full cleanup with vacuum
        octopusos maintenance full-cleanup --vacuum

        # Custom thresholds
        octopusos maintenance full-cleanup --orphan-days 60 --session-days 180
    """
    cleaner = DatabaseCleaner()
