...
from octopusos.core.memory.decay import DecayEngine
from octopusos.core.verify.schema_validator import validate_memory_item
...
def gc_memories(dry_run: bool, decay_rate: float, min_confidence: float, similarity: float):
    ...
    try:
        ...
        console.print("[dim]This is an optional feature requiring the full OctopusOS installation[/dim]")
        raise click.Abort()
...
def memory_health(project_id: Optional[str]):
    ...
    from octopusos.core.memory.budgeter import ContextBudgeter
    ...
    warnings = []
    if expired_count > 0:
        warnings.append(f"{expired_count} memories expired but not cleaned (run 'octopusos memory gc')")
    if budget_pct > 90:
    ...
    if last_gc is None or days_ago > 7:
        warnings.append("GC hasn't run in >7 days, consider running 'octopusos memory gc'")
...
def memory_scope(action, scope_name):
    ...
    Examples:
        octopusos memory scope get
        octopusos memory scope set task
        octopusos memory scope set global
    """
    try:
        from octopusos.core.command import get_registry, CommandContext
