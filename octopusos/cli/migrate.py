...
from octopusos.store.migrations import (
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Database path (defaults to OctopusOS store path)",
)
def migrate(to: Optional[str], db_path: Optional[str]):
    """Migrate database schema to target version.
    
    Examples:
        octopusos migrate              # Migrate to latest version
        octopusos migrate --to 0.8.0   # Migrate to specific version
    """
    if db_path is None:
        db_path = get_db_path()
    else:
