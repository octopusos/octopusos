...
from octopusos.core.git import GitClientWithAuth, CredentialsManager
from octopusos.core.project.repository import ProjectRepository
from octopusos.core.projects.factory import ensure_project_path
from octopusos.schemas.project import RepoRole, RepoSpec
from octopusos.store import get_db, get_db_path
...
from octopusos.cli.commands.project_trace import project_trace
...
def add_project(path: str, project_id: Optional[str]):
    ...
    try:
        ...
        # Insert project
        normalized_path = ensure_project_path(project_id, path=str(project_path))
        cursor.execute(
        ...
        if repo.auth_profile:
            ...
            if not profile:
                errors.append(
                    ...
                    f"Run 'octopusos auth add' to create it."
                )
...
def import_project(
    ...
    Examples:
        ...
        octopusos project import my-app \\
        ...
        octopusos project import --from project.yaml --dry-run

        # Force overwrite existing directories
        octopusos project import --from project.yaml --force
    """
    try:
        # Parse configuration
        if config_file:
            ...
            console.print("\nUsage:")
            console.print("  octopusos project import --from project.yaml")
            console.print("  octopusos project import <name> --repo name=backend,url=...,path=./be")
            raise click.Abort()
        ...
        normalized_path = ensure_project_path(project_name, name=project_name, workspace_root=layout.workspace_root.root_path)
        cursor.execute(
            "INSERT INTO projects (id, path, metadata) VALUES (?, ?, ?)",
            (project_name, normalized_path, json.dumps({"description": description} if description else {})),
        )
        db.commit()
        ...
        if not no_gitignore:
            ...
            from octopusos.core.git import GitignoreManager
        ...
        console.print(Panel.fit(
            ...
            f"Repositories: [cyan]{len(repo_configs)}[/cyan]\n\n"
            f"Next steps:\n"
            f"  • View repos: [yellow]octopusos project repos list {project_name}[/yellow]\n"
            f"  • Validate setup: [yellow]octopusos project validate {project_name}[/yellow]",
...
def list_repos(project_id: str, verbose: bool):
    ...
    \b
    Example:
        octopusos project repos list my-app
        octopusos project repos list my-app --verbose
    """
    try:
        repo_crud = ProjectRepository(get_db_path())
        ...
        if not repos:
            ...
            console.print(f"\nAdd one with: [yellow]octopusos project repos add {project_id}[/yellow]")
            return
...
def add_repo(
    ...
    \b
    Example:
        octopusos project repos add my-app \\
          --name frontend \\
...
def remove_repo(project_id: str, repo_name: str, yes: bool):
    ...
    \b
    Example:
        octopusos project repos remove my-app frontend
        octopusos project repos remove my-app frontend --yes
    """
    try:
        repo_crud = ProjectRepository(get_db_path())
...
def update_repo(
    ...
    \b
    Example:
        octopusos project repos update my-app backend --url git@github.com:org/new-backend
        octopusos project repos update my-app docs --read-only
    """
    try:
        repo_crud = ProjectRepository(get_db_path())
...
def validate_project(
    ...
    Examples:
        # Basic validation
        octopusos project validate my-app

        # Full validation with all checks
        octopusos project validate my-app --all

        # Specific checks
        octopusos project validate my-app --check-urls --check-auth
    """
    try:
        repo_crud = ProjectRepository(get_db_path())
        repos = repo_crud.list_repos(project_id)
...
def check_workspace(project_id: str, workspace_root: Optional[str]):
    ...
    \b
    Example:
        octopusos project workspace check my-app
        octopusos project workspace check my-app --workspace-root /path/to/workspace
    """
    try:
        from octopusos.core.workspace import WorkspaceLayout, WorkspaceValidator
...
def clean_workspace(project_id: str, workspace_root: Optional[str], dry_run: bool, yes: bool):
    ...
    \b
    Example:
        octopusos project workspace clean my-app --dry-run
        octopusos project workspace clean my-app --yes
    """
    try:
        from octopusos.core.workspace import WorkspaceLayout
...
def check_changes(task_id: str, repo_name: Optional[str], strict: bool):
    ...
    Examples:
        ...
        octopusos project check-changes task_01HX123ABC

        # Check changes in specific repo
        octopusos project check-changes task_01HX123ABC --repo backend

        # Use strict security rules
        octopusos project check-changes task_01HX123ABC --strict
    """
    try:
        from octopusos.core.git import ChangeGuardRails, create_strict_guard_rails
        from octopusos.core.task.manager import TaskManager
        from octopusos.core.task.repo_context import TaskRepoContext
        from octopusos.core.project.repository import ProjectRepository
        from octopusos.core.workspace import WorkspaceLayout
