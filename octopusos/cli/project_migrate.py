...
def check_compatibility(project_id: str, check_all: bool):
    ...
    Examples:
        octopusos project migrate check my-project
        octopusos project migrate check --all
    """
    try:
        db = get_db()
        cursor = db.cursor()
...
def migrate_to_multi_repo(project_id: str, workspace_root: str, dry_run: bool):
    ...
    Examples:
        octopusos project migrate to-multi-repo my-project
        octopusos project migrate to-multi-repo my-project --dry-run
        octopusos project migrate to-multi-repo my-project --workspace-root /workspace
    """
    try:
        db = get_db()
        cursor = db.cursor()
        ...
        if success:
            ...
            console.print(Panel(
                ...
                f"Next steps:\n"
                f"  1. Run: octopusos project migrate check {project_id}\n"
...
def list_repos(project_id: str):
    ...
    Examples:
        octopusos project migrate list-repos my-project
    """
    try:
        db = get_db()
        cursor = db.cursor()
...
# This would be done in octopusos/cli/project.py:
# from octopusos.cli.project_migrate import migrate_group
