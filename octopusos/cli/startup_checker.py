...
from octopusos.cli.provider_checker import ProviderChecker
...
class StartupChecker:
    ...
    def _update_provider_config(self, provider_id: str, port: int) -> bool:
        """
        更新 Provider 配置到 ~/.octopusos/config/providers.json
        ...
        try:
            ...
            rprint(f"[dim]配置文件: ~/.octopusos/config/providers.json[/dim]")
    ...
    def prepare_database(self) -> bool:
        ...
        try:
            from octopusos.core.storage.paths import ALLOWED_COMPONENTS, component_db_path
            from octopusos.store import get_migration_status, ensure_migrations, init_db
                ...
                if create:
                    for status in db_status:
                        if not status["exists"]:
                            ...
                            # 为octopusos组件使用init_db，其他组件使用ensure_db_exists
                            if component == "octopusos":
            ...
            # 执行组件级迁移（fail-fast，不允许静默降级）
            for status in db_status:
                component = status["component"]
                try:
                    if component in {"memoryos", "communicationos", "networkos", "brainos"}:
                        from octopusos.core.storage.migrations import ensure_component_migrations
                        ensure_component_migrations(component)
                except Exception as e:
                    rprint(f"[red]✗ {component} 迁移失败: {e}[/red]")
                    logger.error(f"{component} migration bootstrap failed: {e}", exc_info=True)
                    return False

            # 检查octopusos主数据库的迁移状态
            octopusos_db = component_db_path("octopusos")
            return self._check_migrations(octopusos_db)
    ...
    def _check_migrations(self, db_path: Path) -> bool:
        ...
        try:
            from octopusos.store import get_migration_status, ensure_migrations
