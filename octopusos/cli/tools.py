...
def pack_cmd(exec_req_path: str, tool: str, output_path: str):
    ...
    from octopusos.ext.tools import ClaudeCliAdapter, OpenCodeAdapter
...
def auth_cmd(action: str, provider: Optional[str] = None, api_key: Optional[str] = None):
    ...
    Examples:
      octopusos tool auth set --provider lmstudio --api-key lm-studio
      octopusos tool auth status
      octopusos tool auth clear --provider lmstudio
    """
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent.parent))
    from octopusos.core.infra.credentials_manager import CredentialsManager
    from rich.table import Table
    ...
    try:
        ...
        if action == "set":
            ...
            if not providers:
                ...
                console.print("  octopusos tool auth set --provider <name> --api-key <key>")
                return
