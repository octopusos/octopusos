...
class BudgetConfigManager:
    ...
    1. Global: ~/.octopusos/config/budget.json
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        Args:
            config_path: Custom config file path (default: ~/.octopusos/config/budget.json)
        """
        if config_path:
            self.config_path = config_path
        else:
            # Default: ~/.octopusos/config/budget.json
            home = Path.home()
            octopusos_dir = home / ".octopusos"
            config_dir = octopusos_dir / "config"
