...
- Persistent storage (~/.octopusos/runtime/session_context.json)
...
class ContextManager:
    ...
    Features:
    - Persistent storage in ~/.octopusos/runtime/session_context.json
    ...
    def __init__(self, context_file: Optional[Path] = None):
        if context_file is None:
            # Default to ~/.octopusos/runtime/session_context.json
            home = Path.home()
    ...
    def _get_memory_status(self, ctx: Optional[SessionContext]) -> Dict[str, Any]:
        ...
        try:
            memory_dir = Path.home() / ".octopusos" / "memory"
