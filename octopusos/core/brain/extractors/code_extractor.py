...
from octopusos.core.brain.models import Entity, EntityType, Edge, EdgeType, Evidence
...
class CodeExtractor(BaseExtractor):
    ...
    def _should_exclude(self, file_path: Path, repo_path: Path) -> bool:
        ...
        for exclude_pattern in self.exclude_patterns:
            if rel_path.match(exclude_pattern):
                return True
            if exclude_pattern.startswith("**/") and rel_path.match(exclude_pattern[3:]):
                return True
            ...
            for pattern, pattern_type in PYTHON_IMPORT_PATTERNS:
                ...
                if match:
                    if pattern_type == 'from_import':
                        ...
                        # Prefer module path itself if it resolves (common case)
                        module_target = self._resolve_python_import(
                            module_path,
                            file_path,
                            repo_path
                        )
                        ...
                        for name in names:
                            ...
                            if target_file and target_file != module_target:
                                imports.append({
    ...
    def _resolve_python_import(
        ...
        Args:
            import_path: import 路径（如 "octopusos.core.task.manager"）
            source_file: 源文件路径
            repo_path: 仓库根路径

        Returns:
            相对文件路径（如 "octopusos/core/task/manager.py"）或 None
        """
    ...
    def _resolve_js_import(
        ...
        if not target_path.is_relative_to(repo_path):
            # Fallback: treat import as repo-root relative when it escapes
            parts = [p for p in Path(import_path).parts if p not in (".", "..")]
            target_path = (repo_path.joinpath(*parts)).resolve()
