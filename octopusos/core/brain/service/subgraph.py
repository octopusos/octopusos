...
def query_subgraph(
    ...
    try:
        ...
        # Apply min_evidence filter to gap edges (they are suspected by default)
        if min_evidence > 1 and not include_suspected:
            gap_edges = [e for e in gap_edges if e.evidence_count >= min_evidence]
            if gap_edges:
                gap_node_ids = {e.source_id for e in gap_edges} | {e.target_id for e in gap_edges}
                gap_anchors = [n for n in gap_anchors if n.id in gap_node_ids]
            else:
                gap_anchors = []
