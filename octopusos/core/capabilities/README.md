...
The Capability Runner system provides the execution framework for extension capabilities in OctopusOS. It handles routing commands to appropriate executors, managing execution context, and returning formatted results.
...
**Example:**
```python
from octopusos.core.capabilities import CapabilityRunner, CommandRoute, ExecutionContext
...
context = ExecutionContext(
    ...
    user_id="user1",
    extension_id="tools.postman",
    work_dir=Path(".octopusos/extensions/tools.postman/work")
)
...
**Tool Resolution:**
1. `.octopusos/tools/<tool_name>`
2. `.octopusos/bin/<tool_name>`
3. System PATH
...
**Example:**
```python
from octopusos.core.capabilities.response_store import get_response_store
...
```python
from octopusos.core.capabilities import CapabilityRunner, ExecutionContext
from pathlib import Path
...
def execute_extension_capability(route: CommandRoute, session_id: str):
    ...
    context = ExecutionContext(
        ...
        work_dir=Path(f".octopusos/extensions/{route.extension_id}/work/"),
        timeout=300
    )
...
1. **Working Directory Isolation**
   - Commands run in `.octopusos/extensions/<id>/work/`
   - Cannot access files outside work directory

2. **PATH Restrictions**
   - Only `.octopusos/tools/` and `.octopusos/bin/` + system PATH
...
```python
from octopusos.core.capabilities.executors import BaseExecutor
from octopusos.core.capabilities.models import ExecutionResult
...
2. Verify tool exists in `.octopusos/tools/`
