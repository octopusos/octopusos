...
class ShellRunner(Runner):
    ...
    def __init__(
        ...
        if extensions_dir is None:
            extensions_dir = Path.home() / ".octopusos" / "extensions"
    ...
    def run(
        ...
        try:
            ...
            if invocation.timeout:
                timeout = min(invocation.timeout, config.timeout_sec)
    ...
    def _execute_command(
        ...
        try:
            ...
            # Resolve command path if needed (robust to minimal PATH in tests)
            if argv and not Path(argv[0]).is_absolute():
                ...
                if not resolved:
                    for candidate_dir in ("/bin", "/usr/bin", "/usr/local/bin"):
                        candidate = Path(candidate_dir) / argv[0]
                        if candidate.exists():
            ...
            # Guard against known fork-bomb patterns in shell -c commands
            if argv:
                cmd0 = Path(argv[0]).name
                if cmd0 in {"sh", "bash", "zsh"} and "-c" in argv:
                    idx = argv.index("-c")
                    shell_cmd = argv[idx + 1] if idx + 1 < len(argv) else ""
                    if ":(){ :|:& };:" in shell_cmd or ":(){:|:&};:" in shell_cmd:
                        raise RunnerError("Blocked fork bomb pattern in shell command")
            ...
            raise RunnerTimeoutError(
                f"Timeout after {timeout}s: {' '.join(argv)}"
            )
    ...
    def _log_audit_started(
        ...
        """Log audit event for shell execution start"""
        try:
            from octopusos.core.capabilities.audit_events import ExtensionAuditEvent
            from octopusos.core.capabilities.audit_logger import get_audit_logger
