...
class ToolExecutor:
    ...
    Features:
    - Tool path resolution (.octopusos/tools/, system PATH)
    ...
    def _resolve_tool_path(self, tool_name: str) -> Optional[Path]:
        ...
        """
        # Check OctopusOS tool directories first
        ...
        # Fallback: common system paths (robust to minimal PATH)
        for candidate_dir in ("/bin", "/usr/bin", "/usr/local/bin"):
            candidate = Path(candidate_dir) / tool_name
            if candidate.exists() and candidate.is_file() and os.access(candidate, os.X_OK):
                logger.debug(f"Found tool in fallback path: {candidate}")
                return candidate
    ...
    def _is_safe_work_dir(self, work_dir: Path) -> bool:
        ...
        # Must be under .octopusos/ or user's home directory
        octopusos_dir = self.base_dir / ".octopusos"

        try:
            # Try to resolve relative to .octopusos
            work_dir.relative_to(octopusos_dir)
            return True
        except ValueError:
            pass
