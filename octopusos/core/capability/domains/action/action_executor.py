"""
Action Executor for OctopusOS v3
...
from octopusos.core.capability.domains.action.models import (
    ActionExecution,
...
from octopusos.core.capability.domains.action.side_effects_tracker import (
...
class ActionExecutor:
    """
    Core action execution engine for OctopusOS v3.
    ...
    def __init__(
        ...
        self._db_conn = None
        self._action_exec_columns = None
    ...
    def _get_db(self):
        ...
        if self.db_path:
            ...
            if not self._db_conn:
                self._db_conn = sqlite3.connect(self.db_path, timeout=5)
                try:
    ...
    def _get_action_execution_columns(self) -> set:
        """Get column names for action_execution_log table (cached)."""
        if self._action_exec_columns is None:
            try:
                rows = self._execute_sql(
                    "PRAGMA table_info(action_execution_log)"
                ).fetchall()
                self._action_exec_columns = {row[1] for row in rows}
            except Exception:
                self._action_exec_columns = set()
        return self._action_exec_columns
    ...
    def _check_governance(self, execution: ActionExecution):
        ...
        registry = get_capability_registry(db_path=self.db_path)

        # Check if capability exists (map action.execute.* to action.execute)
        capability_id = execution.action_id
        cap_def = registry.get_capability(capability_id)
        if not cap_def and capability_id.startswith("action.execute."):
        ...
        row = self._execute_sql(
            ...
            """,
            (execution.agent_id, execution.action_id, capability_id, utc_now_ms()),
        ).fetchone()
    ...
    def _save_execution(self, execution: ActionExecution):
        ...
        columns = self._get_action_execution_columns()
        has_rollback_plan = "rollback_plan_json" in columns
        ...
        if has_rollback_plan:
            self._execute_sql(
                ...
                (
                    ...
                    json.dumps(execution.rollback_plan)
    ...
    def get_execution(self, execution_id: str) -> Optional[ActionExecution]:
        ...
        if has_rollback_plan:
            row = self._execute_sql(
                ...
                SELECT execution_id, action_id, params_json, decision_id, agent_id,
                       status, result_json, error_message, rollback_plan_json,
                       started_at_ms, completed_at_ms, duration_ms, evidence_id
        ...
        if has_rollback_plan:
            ...
            return ActionExecution(
                ...
                is_reversible=rollback_plan is not None,
                started_at_ms=row[9],
...
def get_action_executor(db_path: Optional[str] = None) -> ActionExecutor:
    ...
    if db_path is not None:
        if _executor_instance is None or _executor_instance.db_path != db_path:
