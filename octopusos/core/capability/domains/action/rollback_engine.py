"""
Rollback Engine for OctopusOS v3
...
class RollbackEngine:
    ...
    def is_reversible(self, execution_id: str) -> bool:
        ...
        effects = execution.actual_side_effects or []

        # If no actual side effects recorded, fall back to prediction
        if not effects:
            try:
                from octopusos.core.capability.domains.action.action_executor import (
                    get_action_executor,
                )

                executor = get_action_executor(db_path=self.db_path)
                effects = executor.side_effects_tracker.predict_side_effects(
                    execution.action_id, execution.params
                )
            except Exception:
                effects = []

        # If we still have no effects and no rollback plan, be conservative
        if not effects:
            return False

        # Check side effects reversibility
