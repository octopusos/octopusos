# Evidence Domain - OctopusOS v3护城河系统
...
Evidence Domain是OctopusOS v3的核心护城河（moat），为监管合规、法律取证和企业审计提供**法庭级别的可靠性**。
...
```python
from octopusos.core.capability.domains.evidence import get_evidence_collector
...
```python
from octopusos.core.capability.domains.evidence import get_evidence_link_graph
...
```python
from octopusos.core.capability.domains.evidence import (
    get_replay_engine,
    ReplayMode,
)
...
# 覆盖率
python3 -m pytest tests/unit/core/capability/evidence/ --cov=octopusos.core.capability.domains.evidence
```
...
3. 运行schema migration：
   ```bash
   sqlite3 store/registry.sqlite < octopusos/store/migrations/schema_v51_evidence_capabilities.sql
   ```
...
1. 检查导出目录权限：
   ```python
   export_dir = Path("/tmp/octopusos_exports")
...
- [Schema v51 Migration](../../../../octopusos/store/migrations/schema_v51_evidence_capabilities.sql)
