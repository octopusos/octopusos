...
from octopusos.core.capability.domains.governance.models import (
    Policy,
    PolicyRule,
...
class PolicyRegistry:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        if _policy_registry_instance is None or _policy_registry_instance.db_path != self.db_path:
            _policy_registry_instance = self

    def _ensure_schema(self) -> None:
        """Ensure governance policy tables exist."""
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='governance_policies'"
        )
    ...
    def load_policy(
        ...
        if cached:
            policy.domain = cached.domain
            policy.name = cached.name
            policy.description = cached.description
            policy.metadata.update(cached.metadata or {})
        self._policies[cache_key] = policy
...
def get_policy_registry(db_path: Optional[str] = None) -> PolicyRegistry:
    ...
    if db_path is not None:
        if _policy_registry_instance is None or _policy_registry_instance.db_path != db_path:
