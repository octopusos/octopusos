"""
Capability Models for OctopusOS v3
...
class CapabilityDomain(str, Enum):
    """
    Five domains in OctopusOS v3 architecture.
...
PermissionLevel = CapabilityLevel
...
def get_default_capabilities() -> List[CapabilityDefinition]:
    """
    Get all 27 default capability definitions for OctopusOS v3.
...
def validate_capability_definition(cap: CapabilityDefinition) -> tuple[bool, Optional[str]]:
    ...
    # Validate capability_id format: domain.operation (legacy) or domain.category.operation
    parts = cap.capability_id.split(".")
    if len(parts) < 2:
        return False, f"capability_id must be in format 'domain.operation' or 'domain.category.operation', got: {cap.capability_id}"
