...
from octopusos.core.chat.commands import CommandResult, get_registry
from octopusos.core.chat.service import ChatService
from octopusos.core.chat.workflow import get_workflow
...
def task_handler(command: str, args: List[str], context: Dict[str, Any]) -> CommandResult:
    ...
    try:
        ...
        message += "```python\n"
        message += "from octopusos.core.task.service import TaskService\n"
        ...
        message += f"```bash\n"
        message += f"octopusos task approve {task.task_id}\n"
        message += "```\n\n"
