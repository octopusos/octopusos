...
from octopusos.core.chat.models.info_need import (
    InfoNeedType,
    ConfidenceLevel,
    DecisionAction,
...
class RuleBasedFilter:
    ...
    # Implicitly time-sensitive topics (no explicit "latest/now" needed)
    IMPLICIT_TIME_KEYWORDS = [
        # Weather
        "weather", "forecast", "temperature", "humidity", "wind", "rain",
        "天气", "气温", "湿度", "风速", "降雨", "降雨概率", "天气预报",
        # Finance
        "exchange rate", "rate", "stock", "stock price", "price", "bitcoin", "gold",
        "汇率", "股票", "股价", "价格", "比特币",
        # Realtime status
        "flight", "flight status", "traffic", "road traffic", "score", "game score", "match",
        "航班", "航班状态", "路况", "比赛", "比分",
    ]
    ...
    def filter(self, message: str) -> ClassificationSignal:
        ...
        # Check implicit time-sensitive keywords
        has_implicit_time = False
        for keyword in self.IMPLICIT_TIME_KEYWORDS:
            if keyword.lower() in message_lower:
                has_implicit_time = True
                matched_keywords.append(f"implicit_time:{keyword}")
        ...
        signal_strength = self._calculate_signal_strength(
            ...
            has_implicit_time=has_implicit_time,
        ...
        return ClassificationSignal(
            ...
            has_implicitly_time_sensitive=has_implicit_time,
...
class InfoNeedClassifier:
    ...
    def _determine_type(self, signals: ClassificationSignal) -> InfoNeedType:
        ...
        if (signals.has_time_sensitive_keywords or signals.has_implicitly_time_sensitive) and \
                signals.has_authoritative_keywords:
            return InfoNeedType.EXTERNAL_FACT_UNCERTAIN

        # Medium signal: Only time-sensitive
        if signals.has_time_sensitive_keywords or signals.has_implicitly_time_sensitive:
            return InfoNeedType.EXTERNAL_FACT_UNCERTAIN
    ...
    def _needs_llm_evaluation(
        ...
        if (preliminary_type == InfoNeedType.EXTERNAL_FACT_UNCERTAIN and
                (signals.signal_strength >= 0.7 or signals.has_implicitly_time_sensitive)):
            return False
    ...
    async def _log_classification_audit(
        ...
        """
        from octopusos.core.audit import log_info_need_classification
