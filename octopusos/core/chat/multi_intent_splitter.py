...
class MultiIntentSplitter:
    ...
    # Connector strength: weak connectors require boundary punctuation to split
    WEAK_CONNECTORS = {
        "zh": {"以及", "还有", "同时", "而且", "并且"},
        "en": {"also", "as well as"},
    }
    ...
    STRONG_CONNECTORS = {
        "zh": {"另外", "顺便", "再者", "此外"},
        "en": {"and also", "additionally", "by the way", "furthermore", "moreover", "besides"},
    }

    # Verbs/markers that indicate a single intent despite connectors
    SINGLE_INTENT_VERBS = {
        "zh": {"比较", "配置", "解释", "说明", "显示", "查看", "列出", "列举", "介绍", "分析", "总结", "定义", "描述"},
        "en": {"explain", "describe", "list", "show", "compare", "define", "analyze", "summarize", "overview", "include"},
    }
    ...
    def split(self, question: str) -> List[SubQuestion]:
        ...
        if len(valid_splits) > self.max_splits:
            if "max_splits" in self.config:
                logger.debug(
                    f"Too many splits ({len(valid_splits)} > {self.max_splits}), not splitting"
                )
                return []
            logger.debug(
                f"Too many splits ({len(valid_splits)} > {self.max_splits}), truncating"
            )
            valid_splits = valid_splits[: self.max_splits]
    ...
    def _split_by_space_enumeration(self, text: str) -> List[SubQuestion]:
        ...
        for i, (start, end, pattern_type) in enumerate(enum_matches):
            ...
            item_text = self._normalize_segment(
                item_text,
                strip_trailing_punct=True,
                strip_leading_connector=False,
            )
    ...
    def _split_by_question_marks(self, text: str) -> List[SubQuestion]:
        ...
        if last_end < len(text):
            ...
            remaining = self._normalize_segment(remaining)
            if remaining:
                splits.append(SubQuestion(
                    text=remaining,
                    index=len(splits),
    ...
    def _split_by_connector(self, text: str) -> List[SubQuestion]:
        ...
        Returns:
            List of SubQuestion objects
        """
        # Find all connector positions with strength
        connectors_found = []
        ...
        for start, end, connector in connectors_found:
            # Determine if connector is strong or weak
            connector_lower = connector.lower()
            is_weak = (
                connector in self.WEAK_CONNECTORS["zh"]
                or connector_lower in self.WEAK_CONNECTORS["en"]
            )
            is_strong = (
                connector in self.STRONG_CONNECTORS["zh"]
                or connector_lower in self.STRONG_CONNECTORS["en"]
            )

            # For weak connectors, require boundary punctuation before splitting
            if is_weak and not self._has_boundary_before(text, start):
                if self._should_block_weak_connector(text):
                    continue

            if not (is_weak or is_strong):
                # Unknown connector type: be conservative
                continue

            # Extract text before connector and strip trailing punctuation
            before = text[last_end:start].strip().rstrip(',，、')
            before = self._normalize_segment(before, strip_leading_connector=False)
            if before:
                splits.append(SubQuestion(
                    text=before,
                    index=len(splits),
        ...
        if last_end < len(text):
            ...
            after = self._normalize_segment(after)
            if after:
                splits.append(SubQuestion(
                    text=after,
                    index=len(splits),
    ...
    def _normalize_segment(
        ...
        strip_leading_connector: bool = True,
    ) -> str:
        """
        Normalize a segment by trimming whitespace, stripping leading connectors,
        and optionally removing trailing punctuation.
        """
        seg = segment.strip()
        if strip_leading_connector:
            seg = self._strip_leading_connector(seg)
        seg = seg.lstrip(',，、;；:：')
        if strip_trailing_punct:
            seg = seg.rstrip('。！？?!;；,，.')
        return seg.strip()

    def _strip_leading_connector(self, text: str) -> str:
        """Remove leading connector words from a segment."""
        if not text:
            return text
        ...
        for conn in leading_connectors:
            if lowered.startswith(conn):
                return text[len(conn):].lstrip()
        return text

    def _has_boundary_before(self, text: str, start: int) -> bool:
        """Check if there is boundary punctuation before position."""
        if start <= 0:
            return False
        boundary_pattern = r'[?？!！。.;；]'
        # Look back for the last non-space char before start
        i = start - 1
        while i >= 0 and text[i].isspace():
            i -= 1
        if i < 0:
            return False
        return re.match(boundary_pattern, text[i]) is not None

    def _should_block_weak_connector(self, text: str) -> bool:
        """Block weak-connector splitting for single-intent expressions."""
        # If question mark present but no boundary, treat as single intent
        if "?" in text or "？" in text:
            return True
        lowered = text.lower()
        for verb in self.SINGLE_INTENT_VERBS["en"]:
            if verb in lowered:
                return True
        for verb in self.SINGLE_INTENT_VERBS["zh"]:
            if verb in text:
                return True
        return False
