...
from octopusos.core.chat.models import ChatSession, ChatMessage, ConversationMode
from octopusos.core.db import registry_db
from octopusos.core.chat.session_recording import RecordingPayload, SessionRecordingSink
from octopusos.core.chat.event_ledger import append_observed_event
from octopusos.core.chat.writer_mode import get_chat_writer_mode, is_ledger_enabled
...
def validate_message_role(role: str) -> str:
    ...
    and data integrity issues. Only 'user', 'assistant', 'system', and 'tool' roles
...
class ChatService:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        Args:
            db_path: Path to database file (defaults to OctopusOS registry DB)
        ...
        if db_path is not None:
            logger.warning(
                ...
                "All DB access now goes through octopusos.core.db.registry_db"
            )
        recordings_root = os.getenv("OCTOPUSOS_SESSION_RECORDINGS_ROOT", "recordings/sessions")
        self._session_recording_sink = SessionRecordingSink(root_dir=recordings_root)
        self._session_writer: Optional[SessionWriter] = None
        self._session_writer_lock = threading.Lock()

    def _get_session_writer(self) -> SessionWriter:
    ...
    def cancel_session_run(
        ...
        *,
        ...
        actor: str = "ws",
        cancel_run: Optional[Callable[[], Dict[str, Any]]] = None,
        cancel_hold: Optional[Callable[[], Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        """Unified cancellation entrypoint for chat run and hold scopes."""
        allowed_scopes = {"run_only", "hold_only", "run_and_hold"}
        ...
        run_result: Dict[str, Any] = {"ok": True, "status": "skipped", "scope": "run"}
        hold_result: Dict[str, Any] = {"ok": True, "status": "skipped", "scope": "hold"}

        if scope in {"run_only", "run_and_hold"} and cancel_run:
            run_result = cancel_run()
        if scope in {"hold_only", "run_and_hold"} and cancel_hold:
            hold_result = cancel_hold()

        return {
            "ok": bool(run_result.get("ok", True)) and bool(hold_result.get("ok", True)),
            "session_id": session_id,
            "command_id": command_id,
            "run_id": run_id,
            "reason": reason,
    ...
    def create_session(
        ...
        try:
            ...
            if get_chat_writer_mode() == "dual_write" and is_ledger_enabled():
    ...
    def update_session_metadata(
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE session_id = ?
                """,
                (json.dumps(current_metadata), updated_at.strftime("%Y-%m-%d %H:%M:%S"), now, now, session_id)
            )
            conn.commit()
    ...
    def update_session_title(
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE session_id = ?
                """,
                (title, updated_at.strftime("%Y-%m-%d %H:%M:%S"), now, now, session_id)
            )
            conn.commit()
    ...
    def add_message(
        ...
        *,
        ...
        Args:
            session_id: Session ID
            role: Message role (user/assistant/system/tool only - validated strictly)
        ...
        # Resolve idempotency key (prefer explicit param over metadata).
        effective_idempotency_key: Optional[str] = None
        if isinstance(idempotency_key, str) and idempotency_key.strip():
            effective_idempotency_key = idempotency_key.strip()
        else:
            raw_key = metadata.get("idempotency_key")
            if isinstance(raw_key, str) and raw_key.strip():
                effective_idempotency_key = raw_key.strip()

        mode = get_chat_writer_mode()
        if mode == "ledger_primary" and role == "user" and effective_idempotency_key:
            outcome = self._get_session_writer().apply_user_message_requested(
                session_id=session_id,
                content=content,
            ...
            return self.get_message(outcome.message_id)

        conn = self._get_conn()
        cursor = conn.cursor()

        try:
            # Phase 1: legacy is main write-path; ledger is best-effort sidecar.
            # Redline: dual_write must NOT involve SessionWriter apply (takeover only in ledger_primary).
            if mode == "dual_write" and is_ledger_enabled() and role == "user":
                append_observed_event(
                    session_id=session_id,
                    event_type="user_message_requested",
                    source=source,
                    payload={
                        "role": role,
                        "content": content,
            ...
            cursor.execute(
                ...
                WHERE session_id = ?
                """,
                (created_at.strftime("%Y-%m-%d %H:%M:%S"), now, now, session_id)
            )
    ...
    def _record_message_to_session_async(self, message: ChatMessage) -> None:
        """Record persisted message to append-only session markdown (sidecar)."""
        try:
            ...
            self._session_recording_sink.enqueue(self, payload)
        except Exception as recording_err:
            # Sidecar only: message persistence must remain authoritative
            logger.warning(f"Session recording enqueue failed: {recording_err}")
