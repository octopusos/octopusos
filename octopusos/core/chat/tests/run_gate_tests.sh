...
while [[ $# -gt 0 ]]; do
    case $1 in
        --coverage)
            COVERAGE="--cov=octopusos.core.chat.comm_commands --cov=octopusos.core.chat.communication_adapter --cov=octopusos.core.chat.guards --cov-report=term-missing"
            shift
            ;;
        --quiet)
            VERBOSE=""
...
# Navigate up to project root: tests -> chat -> core -> octopusos -> root
...
# Run tests
TEST_FILE="octopusos/core/chat/tests/test_comm_integration_gates.py${CATEGORY}"
