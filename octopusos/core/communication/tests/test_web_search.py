...
from octopusos.core.communication.connectors.html_search_client import BlockedError as HtmlBlockedError
...
class TestWebSearchConnector:
    ...
    def test_validate_config_invalid_google_mode(self):
        """Test configuration validation with invalid google mode."""
        connector = WebSearchConnector({"engine": "google", "google_mode": "invalid"})
        assert connector.validate_config() is False

    @patch("octopusos.core.communication.connectors.web_search.importlib.import_module")
    def test_extension_entrypoint_loads_backend(self, mock_import_module):
        """Test extension backend can be loaded via entrypoint."""
        module = Mock()
        backend = Mock()
        module.create_backend = Mock(return_value=backend)
        mock_import_module.return_value = module

        connector = WebSearchConnector({
            "engine": "duckduckgo",
            "extension_entrypoint": "fake.module:create_backend",
        })

        assert connector.search_backend is backend
        module.create_backend.assert_called_once()

    @patch("octopusos.core.communication.connectors.web_search.importlib.import_module")
    @pytest.mark.asyncio
    async def test_disabled_fallback_when_extension_unavailable(self, mock_import_module):
        """Test clear disabled fallback when extension backend is unavailable."""
        mock_import_module.side_effect = ImportError("not installed")
        connector = WebSearchConnector({
            "engine": "duckduckgo",
            "extension_entrypoint": "missing.module:create_backend",
        })

        with pytest.raises(APIError, match="backend is disabled"):
            await connector.execute("search", {"query": "sydney weather"})
    ...
    async def test_search_duckduckgo_success(self, mock_search):
        ...
        assert result["results"][0]["source"] == "duckduckgo_raw"
    ...
    @pytest.mark.asyncio
    @patch.object(WebSearchConnector, "_search_google")
    async def test_search_google_success(self, mock_search):
        """Test Google search route."""
    ...
    @pytest.mark.asyncio
    @patch.object(WebSearchConnector, "_search_bing")
    async def test_search_bing_success(self, mock_search):
        """Test Bing search route."""
        mock_search.return_value = [
            {
                "title": "Bing Result",
    ...
    @pytest.mark.asyncio
    async def test_search_blocked_error(self):
        """Test blocked error from HTML client is preserved."""
        connector = WebSearchConnector({"engine": "google"})
        connector.search_backend = Mock(side_effect=HtmlBlockedError("challenge"))  # type: ignore[assignment]
        connector.search_backend.search = Mock(side_effect=HtmlBlockedError("challenge"))  # type: ignore[attr-defined]
        with pytest.raises(BlockedError, match="challenge"):
            await connector.execute("search", {"query": "test"})

    @pytest.mark.asyncio
    async def test_search_success_cache_hit(self):
        """Test successful responses are served from cache."""
        connector = WebSearchConnector({"engine": "duckduckgo", "success_cache_ttl": 60})
        connector._search_duckduckgo = AsyncMock(return_value=[  # type: ignore[method-assign]
            {"title": "Result 1", "href": "https://example.com/page1", "body": "snippet"}
        ])

        first = await connector.execute("search", {"query": "cached query"})
        second = await connector.execute("search", {"query": "cached query"})

        assert first["total_results"] == 1
        assert second["total_results"] == 1
        connector._search_duckduckgo.assert_awaited_once()  # type: ignore[attr-defined]

    @pytest.mark.asyncio
    async def test_search_failure_cache_hit(self):
        """Test failed responses are cached briefly."""
        connector = WebSearchConnector({"engine": "duckduckgo", "failure_cache_ttl": 60})
        connector._search_duckduckgo = AsyncMock(side_effect=NetworkError("boom"))  # type: ignore[method-assign]

        with pytest.raises(NetworkError, match="boom"):
            await connector.execute("search", {"query": "bad query"})
        with pytest.raises(NetworkError, match="boom"):
            await connector.execute("search", {"query": "bad query"})
...
class TestResultStandardization:
    ...
    def test_standardize_duckduckgo_format(self):
        ...
        assert standardized[0]["source"] == "duckduckgo_raw"
        assert standardized[0]["published_at"] == ""
...
class TestResultDeduplication:
    ...
    def test_deduplicate_removes_tracking_params(self):
        """Test deduplication strips tracking params when comparing URLs."""
        results = [
            {"title": "Result 1", "url": "https://example.com/page?utm_source=x", "snippet": "Test"},
            {"title": "Result 2", "url": "https://example.com/page", "snippet": "Test"},
        ]
        deduplicated = self.connector._deduplicate_results(results)
        assert len(deduplicated) == 1
...
class TestDuckDuckGoSearch:
    ...
    @pytest.mark.asyncio
    @patch("asyncio.get_event_loop")
    async def test_duckduckgo_search_delegates_to_html_client(self, mock_get_event_loop):
        """Test DuckDuckGo search delegates to extension backend via executor."""
        backend = Mock()
        self.connector.search_backend = backend
        mock_loop = Mock()
        ...
        results = await self.connector._search_duckduckgo("test query", 10, "en")
...
class TestPriorityScoring:
    ...
    def test_priority_score_handles_scoring_error(self):
        ...
        with patch("octopusos.core.communication.connectors.web_search.calculate_priority_score") as mock_score:
    ...
    def test_trusted_sources_fallback_on_error(self):
        ...
        with patch("octopusos.core.communication.connectors.web_search.load_trusted_sources") as mock_load:
