"""
Centralized Configuration Management for OctopusOS
...
Usage:
    from octopusos.core.config import get_config
...
class OctopusOSConfig(BaseSettings):
    """
    Central configuration for OctopusOS

    All settings can be overridden via environment variables with OCTOPUSOS_ prefix.
    For example: OCTOPUSOS_DATABASE_URL, OCTOPUSOS_SECRET_KEY, etc.
    """
    ...
    _database_url: Optional[str] = Field(
        default=None,
        alias="database_url",
        description="Database connection URL (override via OCTOPUSOS_DATABASE_URL)"
    )
    ...
    def database_url(self) -> str:
        ...
        path = component_db_path("octopusos")
    ...
    class Config:
        env_file = ".env"
        env_prefix = "OCTOPUSOS_"
        case_sensitive = False


# Global config instance
_config: Optional[OctopusOSConfig] = None


def get_config(force_reload: bool = False) -> OctopusOSConfig:
    """
