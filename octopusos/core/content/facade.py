...
from octopusos.core.db.row_utils import row_to_dict
...
class UnifiedContentFacade:
    ...
    def __init__(self, registry: Optional[ContentRegistry] = None, db_path: Optional[Path] = None):
        ...
        Args:
            ...
            db_path: Path to database file (defaults to component_db_path("octopusos"))
        """
    ...
    def _get_connection(self) -> sqlite3.Connection:
        ...
        if not self.db_path.exists():
            raise FileNotFoundError(
                f"Database not initialized. Run 'octopusos init' and 'octopusos migrate --to 0.5.0' first."
            )
    ...
    def _list_from_policy_lineage(self, filters: dict, limit: int) -> list[dict]:
        ...
        return [
            ...
            for row_dict in (row_to_dict(row) for row in rows)
        ]
        ...
        return [
            {
                ...
                "spec": {
                    "memory_type": row_dict.get("type"),
                    "content": json.loads(row_dict["content"]) if row_dict.get("content") else None,
                },
            }
            for row_dict in (row_to_dict(row) for row in rows)
        ]
