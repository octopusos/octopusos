"""Database configuration and engine management for OctopusOS.
...
Usage:
    ...
    from octopusos.core.database import get_engine
...
Environment Variables:
    ...
    DATABASE_NAME: Database name, default: octopusos
    DATABASE_USER: PostgreSQL username, default: postgres
    DATABASE_PASSWORD: PostgreSQL password, default: empty
    SQLITE_PATH: SQLite database file path (DEPRECATED, use OCTOPUSOS_DB_PATH)
    OCTOPUSOS_DB_PATH: SQLite database file path (preferred)
"""
...
class DatabaseConfig:
    ...
    def __init__(self):
        ...
        self.db_name = os.getenv("DATABASE_NAME", "octopusos")
        ...
        # 2. OCTOPUSOS_DB_PATH (preferred environment variable)
        # 3. Unified path from storage.paths module (default)
        env_path = os.getenv("SQLITE_PATH") or os.getenv("OCTOPUSOS_DB_PATH")
        if env_path:
            self.sqlite_path = env_path
            logger.info(f"SQLite path from environment: {self.sqlite_path}")
        else:
            self.sqlite_path = str(component_db_path("octopusos"))
