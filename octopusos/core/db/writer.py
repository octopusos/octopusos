...
Usage Example:
    ...
    writer = SQLiteWriter(db_path=str(component_db_path("octopusos")))
...
class SQLiteWriter:
    ...
    def __new__(cls, db_path: str, **kwargs):
        ...
        with cls._lock:
            if db_path not in cls._instances:
                ...
                # Guard __init__ so concurrent callers don't start multiple threads.
    ...
    def __init__(
        ...
        """
        # Avoid re-initialization for singleton; also protect against concurrent
        # initialization racing on the same singleton instance.
    ...
    def _open(self) -> Connection:
        ...
        for attempt in range(10):
            try:
                # Use sqlite3 module's implicit transaction handling with IMMEDIATE mode.
                # This avoids explicit BEGIN nesting edge cases under high concurrency while
                # still acquiring the write lock early when a transaction starts.
                conn = sqlite3.connect(self.db_path, check_same_thread=False, isolation_level="IMMEDIATE")
        ...
        raise sqlite3.OperationalError(
            f"Failed to open database {self.db_path}: {last_error}"
        )
    ...
    def _exec_with_retry(
        ...
        def _best_effort_rollback(c: Connection) -> None:
            try:
                c.rollback()
            except Exception:
                pass
        ...
        for attempt in range(max_retry):
            try:
                ...
                # Defensive cleanup: the writer thread should not carry transactions across jobs.
                if getattr(conn, "in_transaction", False):
                    _best_effort_rollback(conn)
