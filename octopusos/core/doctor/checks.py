...
def check_basic_imports() -> CheckResult:
    ...
    try:
        ...
        result = subprocess.run(
            ["uv", "run", "python", "-c", "import octopusos; import fastapi; import rich"],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def check_networkos_db() -> CheckResult:
    ...
    try:
        from octopusos.networkos.health import NetworkOSHealthCheck
...
def check_db_schema_consistency() -> CheckResult:
    """Check that current DB tables match a freshly migrated schema."""
    try:
        from octopusos.core.storage.paths import component_db_path
        from octopusos.store.migrator import auto_migrate

        current_db = component_db_path("octopusos")
        if not current_db.exists():
            return CheckResult(
                name="db-schema",
                status=CheckStatus.WARN,
                summary="主数据库未初始化，跳过表完整性校验",
                details=["运行 octopusos init 后再检查"],
            )

        with tempfile.TemporaryDirectory() as temp_dir:
            fresh_db = Path(temp_dir) / "fresh.sqlite"
            ...
            auto_migrate(fresh_db)

            conn_f = sqlite3.connect(str(fresh_db))
            fresh_tables = {row[0] for row in conn_f.execute("SELECT name FROM sqlite_master WHERE type='table'")}
            conn_f.close()

        conn_c = sqlite3.connect(str(current_db))
        current_tables = {row[0] for row in conn_c.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        conn_c.close()

        missing_tables = sorted(fresh_tables - current_tables)
        extra_tables = sorted(current_tables - fresh_tables)

        if missing_tables:
            details = [
                f"缺失表数量: {len(missing_tables)}",
                f"示例: {', '.join(missing_tables[:8])}",
            ]
            return CheckResult(
                name="db-schema",
                status=CheckStatus.FAIL,
        ...
        if extra_tables:
            details = [
                f"检测到额外表数量: {len(extra_tables)}",
                f"示例: {', '.join(extra_tables[:8])}",
                "这通常是兼容/历史表，不影响运行",
            ]
            return CheckResult(
                name="db-schema",
                status=CheckStatus.WARN,
                summary="数据库存在额外表（非迁移基线）",
                details=details,
            )
        ...
        return CheckResult(
            name="db-schema",
            status=CheckStatus.PASS,
            summary=f"数据库表完整性通过 ({len(current_tables)} tables)",
        )
    except Exception as e:
        return CheckResult(
            name="db-schema",
