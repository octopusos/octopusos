...
from octopusos.core.mode.mode import get_mode, ModeViolationError
from octopusos.core.mode.mode_alerts import alert_mode_violation, AlertSeverity
# Task 27: Mode Event Listener integration
from octopusos.core.mode.mode_event_listener import emit_mode_violation

# Task-Driven: Import TaskManager
from octopusos.core.task import TaskManager

# Task #3: Planning Guard - v0.6 Soul
from octopusos.core.task.planning_guard import get_planning_guard
from octopusos.core.task.errors import PlanningSideEffectForbiddenError
...
class ExecutorEngine:
    ...
    def execute(
        ...
        if caller_source == "chat":
            from octopusos.core.task.errors import ChatExecutionForbiddenError
        ...
        if not task_id:
            ...
            run_tape.audit_logger.log_event(
                event_type="warning",
                details={
                    "event": "execution_without_task_id",
        ...
        if not task:
            # Fallback: create orphan task instead of failing hard
            run_tape.audit_logger.log_event(
                event_type="warning",
                details={
                    "event": "execution_task_missing",
                    "execution_request_id": exec_req_id,
                    "task_id": task_id,
                    "action": "creating_orphan_task",
                },
        ...
        # Task #4: Check spec_frozen flag (planning stage is allowed before freeze)
        mode_id = execution_request.get("mode_id")
        if mode_id != "planning" and not task.is_spec_frozen() and not task.is_orphan() and not orphan_created:
            # Allow task_runner to proceed for early-stage tasks to avoid false negatives
            if caller_source == "task_runner" and task.status in {"created", "draft", "planning"}:
                run_tape.audit_logger.log_event("execution_warning_spec_not_frozen", details={
                    "task_id": task_id,
                    "spec_frozen": task.spec_frozen,
                    "reason": "Execution proceeded for early-stage task without frozen spec",
                    "caller_source": caller_source,
                    "enforcement": "task_4_frozen_plan_validation",
                    "v06_constraint": True
                })
            else:
                from octopusos.core.task.errors import SpecNotFrozenError
        ...
        # Determine if planning guard should be skipped for early-stage tasks
        skip_planning_guard = False
        if caller_source == "task_runner" and task.status in {"created", "draft", "planning", "orphan"}:
            skip_planning_guard = True
        ...
        # 
        # 验收命令:
        #   rg "get_mode\(" octopusos/core/executor | wc -l
                ...
                for step in steps:
                    for op in step.get("operations", []):
                        ...
                        result = self._execute_operation(
                            op,
                            worktree_path,
                            skip_planning_guard=skip_planning_guard,
                        )
    ...
    def apply_diff_or_raise(
        ...
        #
        # 验收命令:
        #   rg "apply_diff_or_raise" octopusos | wc -l
        #   期望结果: 2（定义 + 调用）
        #
        #   rg "GitClient\.apply_patch\(" octopusos | wc -l
        #   期望结果: 2（定义 + 在本方法内调用）
        #
        # Mode 检查硬约束:
