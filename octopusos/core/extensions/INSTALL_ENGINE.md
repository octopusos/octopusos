...
- Working directory: `.octopusos/extensions/<extension_id>/work/`
- PATH: System paths + `~/.octopusos/bin/`
...
**Storage:** Config is saved to `.octopusos/extensions/<extension_id>/work/config.json`
...
steps:
  ...
  - id: install_postman_cli_linux
    ...
    command: |
      ...
      tar -xzf /tmp/postman.tar.gz -C ~/.octopusos/bin/
  ...
  - id: save_config
    ...
    config_value: ~/.octopusos/bin/postman
...
from octopusos.core.extensions import ExtensionInstallEngine, ExtensionRegistry
...
result = engine.execute_install(
    ...
    plan_yaml_path=Path(".octopusos/extensions/tools.postman/install/plan.yaml"),
...
- **Restricted PATH**: Only system paths + `~/.octopusos/bin/`
- **Restricted ENV**: Whitelist-only environment variables
- **Working Directory**: Scoped to `.octopusos/extensions/<extension_id>/work/`
...
**Solution**: Ensure command is in PATH or add to `~/.octopusos/bin/`:

```yaml
- id: add_to_path
  type: exec.shell
  command: |
    ln -s /opt/tool/bin/tool ~/.octopusos/bin/tool
```
