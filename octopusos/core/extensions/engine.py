...
from octopusos.core.extensions.exceptions import InstallationError
from octopusos.core.extensions.models import InstallStatus
        ...
        try:
            ...
            eval_namespace = {
                ...
                'platform': SimpleNamespace(
                    os=context.platform_os,
                    arch=context.platform_arch,
                ),
            }
...
class SandboxedExecutor:
    ...
    def _build_environment(self, env_whitelist: Optional[List[str]] = None) -> Dict[str, str]:
        ...
        # Restrict PATH
        octopusos_bin = Path.home() / ".octopusos" / "bin"
        system_paths = [
            "/usr/local/bin",
            "/usr/bin",
            "/bin",
        ...
        if octopusos_bin.exists():
            system_paths.insert(0, str(octopusos_bin))

        env["PATH"] = ":".join(system_paths)

        # Add extension-specific vars
        env["OCTOPUSOS_EXTENSION_ID"] = self.extension_id
        env["OCTOPUSOS_WORK_DIR"] = str(self.work_dir)
...
class ExtensionInstallEngine:
    ...
    def execute_install(
        ...
        try:
            ...
            # Setup context
            work_dir = Path.home() / ".octopusos" / "extensions" / extension_id / "work"
    ...
    def _log_step_execution(
        ...
        try:
            ...
            try:
                from octopusos.core.audit import log_audit_event, EXTENSION_STEP_EXECUTED
