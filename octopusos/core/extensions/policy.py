...
class PolicyChecker:
    """
    PolicyChecker enforces OctopusOS governance policies for extensions.
    ...
    def check_runtime_policy(manifest: dict) -> PolicyCheckResult:
        ...
        if runtime != "python":
            return PolicyCheckResult(
                allowed=False,
                reason="Extension requires external binary runtime which violates OctopusOS governance policy. Only Python runtime extensions are currently supported.",
