...
from octopusos.core.extensions.exceptions import RegistryError
...
class ExtensionRegistry:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        Args:
            db_path: Database file path (optional, defaults to OctopusOS registry)
        """
        if db_path is None:
            from octopusos.store import get_db_path, ensure_migrations
            db_path = get_db_path()
    ...
    def _execute_write(self, operation_fn, *args, **kwargs):
        ...
        try:
            ...
            if self.db_path == get_db_path():
                ...
                from octopusos.store import get_writer
                writer = get_writer()
            ...
            try:
                ...
                if existing:
                    ...
                    if existing['status'] in ('INSTALLING', 'FAILED', 'UNINSTALLED'):
                        ...
                        # Extension is properly installed - duplicate registration is not allowed
                        raise RegistryError(
                            f"Extension '{manifest.id}' is already installed. "
                            "Uninstall it before re-registering."
                        )
                    else:
