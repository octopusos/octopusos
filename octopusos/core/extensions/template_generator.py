...
# All commands are executed by OctopusOS Core - no custom code execution.
#
# Generated by OctopusOS Extension Wizard
...
This is a **Declarative-Only Extension** following OctopusOS ADR-EXT-001.

### What This Means

- **No executable code**: This extension does not contain Python, JavaScript, or shell scripts
- **Core-controlled execution**: All commands are executed by OctopusOS Core in a sandboxed environment
- **Security**: Cannot execute arbitrary code or bypass OctopusOS security boundaries

See `docs/DESIGN.md` for detailed architecture documentation.

## Development

This extension was generated using the OctopusOS Extension Wizard.
...
3. Install via WebUI
4. Test commands in OctopusOS chat interface
...
```
# Combine with other OctopusOS features
$advanced_example
```
...
**Solution**: Check the execution logs in the OctopusOS WebUI for error details
...
This extension follows OctopusOS ADR-EXT-001: **Declarative Extensions Only**.
...
Unlike traditional plugin systems, OctopusOS extensions do NOT require a `handlers.py` or similar executable code file. All command execution is handled by:
...
1. OctopusOS Core reads `commands/commands.yaml`
...
- Extension Validator: `octopusos.core.extensions.validator`
- Capability Runner: `octopusos.core.capabilities.runner`
- Security Model: See ADR-EXT-001-ENFORCEMENT.md

## Version

Generated with OctopusOS Extension Wizard v1.0 (Task #14: Declarative-Only)
''')
    ...
    def _generate_usage_instructions(
        ...
        for cap in capabilities:
            if cap["type"] == "slash_command":
                instructions.append(
                    f"### Using {cap['name']}\n\n"
                    f"In the OctopusOS chat interface, type:\n\n"
                    f"```\n{cap['name']} [arguments]\n```\n"
                )
