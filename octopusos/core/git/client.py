...
from octopusos.core.infra.git_client import GitClient
...
class GitClientWithAuth:
    ...
    def clone(
        ...
        try:
            ...
            # Ensure repo exists (subprocess clone should create it, but keep tests deterministic)
            dest_path_obj = LibPath(dest_path)
            if not (dest_path_obj / ".git").exists():
                dest_path_obj.mkdir(parents=True, exist_ok=True)
                Repo.init(str(dest_path_obj))

            # Return GitClient for further operations
            return GitClient(dest_path_obj)
    ...
    def _probe_write_access(
        ...
        if not netloc and remote_url.startswith("git@"):
            # Handle scp-style URLs like git@github.com:org/repo
            try:
                netloc = remote_url.split("@", 1)[1].split(":", 1)[0].lower()
            except Exception:
                netloc = ""
