...
- Protected system files (.git/, OctopusOS config)
...
class ChangeGuardRails:
    ...
    DEFAULT_FORBIDDEN_PATTERNS = [
        ...
        # OctopusOS config (protected unless explicitly allowed)
        r"\.octopusos/config\.ya?ml$",
        r"\.octopusos/credentials\.db$",
    ...
    def _get_forbidden_file_hint(self, file_path: str) -> str:
        ...
        if ".octopusos/" in normalized:
            return (
                "OctopusOS configuration files are protected. "
                "Modify them through octopusos CLI instead."
            )
