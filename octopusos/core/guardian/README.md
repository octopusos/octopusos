...
Guardian 是 OctopusOS 的验收事实记录器（Verification / Acceptance Authority）。
...
```python
from octopusos.core.guardian import GuardianReview
...
```python
from octopusos.core.guardian import GuardianStorage
...
```python
from octopusos.core.guardian.policies import GuardianPolicy
...
```python
from octopusos.core.guardian.policies import get_policy_registry
...
Guardian 提供完整的 REST API（详见 `octopusos/webui/api/guardian.py`）。
...
```python
from octopusos.core.task import TaskService
...
Guardian 使用 `guardian_reviews` 表存储验收记录（详见 `octopusos/store/migrations/v22_guardian_reviews.sql`）。
...
# 查看覆盖率
pytest tests/unit/guardian/ --cov=octopusos.core.guardian --cov-report=term-missing
```
