...
#### 1.1 Job 脚本
**文件**: `/octopusos/jobs/lead_scan.py`
...
- **并发保护**：
  ...
  - 使用 `/tmp/octopusos_lead_scan.lock` 文件锁
  - 支持 `--force` 参数跳过并发检查
...
```bash
# 帮助信息
$ python -m octopusos.jobs.lead_scan --help
✅ 显示完整帮助信息

# Dry-run 模式
$ python -m octopusos.jobs.lead_scan --window 24h --dry-run
✅ 成功运行，输出统计信息
✅ 显示 "DRY RUN - No tasks were created"

# 实际运行
$ python -m octopusos.jobs.lead_scan --window 7d
...
```bash
# Terminal 1
$ python -m octopusos.jobs.lead_scan --window 24h
# (运行中...)
...
# 强制运行
$ python -m octopusos.jobs.lead_scan --window 24h --force
✅ 跳过并发检查，强制运行
```
...
```bash
# 测试：数据库不存在
$ python -m octopusos.jobs.lead_scan --db-path /nonexistent/db.db --window 24h
✅ 捕获异常，打印错误，退出码 1
✅ 不抛出未处理异常

# 测试：无效窗口
$ python -m octopusos.jobs.lead_scan --window invalid
✅ 显示错误信息，退出码 1
```
...
```cron
# 每天凌晨 2 点运行 24h 扫描
0 2 * * * /usr/bin/python3 -m octopusos.jobs.lead_scan --window 24h >> /var/log/octopusos/lead_scan_24h.log 2>&1

# 每周一凌晨 3 点运行 7d 扫描
0 3 * * 1 /usr/bin/python3 -m octopusos.jobs.lead_scan --window 7d >> /var/log/octopusos/lead_scan_7d.log 2>&1
```
...
| 组件 | 文件 | 用途 |
|------|------|------|
| LeadStorage | `octopusos/core/lead/adapters/storage.py` | 查询 Supervisor 决策数据 |
| RiskMiner | `octopusos/core/lead/miner.py` | 执行 6 条规则检测 |
| LeadFindingStore | `octopusos/core/lead/dedupe.py` | 去重存储 findings |
| LeadTaskCreator | `octopusos/core/lead/adapters/task_creator.py` | 创建 follow-up tasks |
...
- **锁文件未释放**：
  ```bash
  rm /tmp/octopusos_lead_scan.lock
  # 或使用 --force 强制运行
  ```

- **数据库迁移**：
  ```bash
  sqlite3 ~/.octopusos/store.db < octopusos/store/migrations/v14_supervisor.sql
  ```
...
- [Memory GC Job](../../octopusos/jobs/memory_gc.py)（参考实现）
...
- [x] 实现 `octopusos/jobs/lead_scan.py`
- [x] 实现并发保护（文件锁）
