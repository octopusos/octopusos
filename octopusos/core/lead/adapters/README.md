...
from octopusos.core.lead.dedupe import LeadFinding, LeadFindingStore
...
`LeadStorage` 提供只读查询接口，从 OctopusOS 数据库提取数据：
...
- `octopusos.core.task.service`: TaskService（创建任务）
- `octopusos.core.task.states`: TaskState（状态管理）
- `octopusos.core.lead.dedupe`: LeadFindingStore（去重存储）
- `octopusos.core.lead.models`: LeadFinding（领域模型）
