...
只读查询适配器，从 OctopusOS 数据库（task_audits, tasks 等表）提取 Supervisor 决策和指标数据。
...
from octopusos.core.lead.models import ScanWindow
...
class LeadStorage:
    ...
    def get_decision_lag(self, window: ScanWindow) -> Dict[str, Any]:
        ...
        try:
            ...
            has_decision_id = 'decision_id' in columns
            has_payload = 'payload' in columns

            lags = []

            if has_redundant_columns and has_payload:
                # v21+ 路径：使用冗余列（性能优化）
                decision_id_col = "decision_id" if has_decision_id else "NULL AS decision_id"
                cursor.execute(
                    """
                    SELECT
                        {decision_id_col},
                    ...
                    WHERE event_type LIKE 'SUPERVISOR_%'
                      ...
                      AND created_at <= ?
                    ORDER BY created_at DESC
                    """.format(decision_id_col=decision_id_col),
                ...
                cursor.execute(
                    """
                    SELECT {decision_id_col}, payload, created_at
                    FROM task_audits
                    WHERE event_type LIKE 'SUPERVISOR_%'
                      AND created_at >= ?
                      AND created_at <= ?
