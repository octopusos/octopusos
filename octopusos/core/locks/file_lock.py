...
@dataclass(frozen=True)
class AcquireBatchResult:
    """Compatibility return type for FileLock.acquire_batch."""
...
class FileLock:
    ...
    def acquire_batch(
        ...
        Args:
            ...
            holder: Holder identifier (optional for legacy callers)
        ...
        Returns:
            AcquireBatchResult with success flag and locked paths
        """
        actual_task_id = task_id or self.task_id or "unknown"
        actual_repo_root = repo_root or self._repo_root
        actual_holder = holder
        if actual_holder is None:
            if run_id is not None:
                actual_holder = f"run:{run_id}"
            elif task_id is not None:
                actual_holder = f"task:{task_id}"
            elif self.run_id is not None:
                actual_holder = f"run:{self.run_id}"
            elif self.task_id is not None:
                actual_holder = f"task:{self.task_id}"
            else:
                actual_holder = "legacy"
        ...
        try:
            ...
            return AcquireBatchResult(success=True, locked=[])
        except LockConflict:
            locked: list[str] = []
            try:
                conn = self._mgr._get_connection()
                cursor = conn.cursor()
                locked = self._mgr._check_locked(cursor, actual_repo_root, file_paths, utc_now())
            except Exception:
                locked = []
            finally:
                try:
                    ...
                    pass
            return AcquireBatchResult(success=False, locked=locked)
    ...
    def release_batch(
        ...
        """
        # Backward-compatible argument order: (repo_root, file_paths, run_id)
        if isinstance(file_paths, str) and isinstance(repo_root, list):
            file_paths, repo_root = repo_root, file_paths
