...
class TaskLock:
    ...
    def acquire(
        ...
        def _looks_like_task_id(value: str) -> bool:
            ...
            return value_lower.startswith("task") or value_lower.startswith("t-") or "task" in value_lower

        # Handle parameter aliases for backward compatibility
        actual_duration = duration or lease_duration

        if task_id is not None and worker_id is None and isinstance(holder, str) and isinstance(task_id, str):
            # Legacy positional order: (task_id, run_id, worker_id)
            if _looks_like_task_id(holder) and not _looks_like_task_id(task_id):
                actual_task_id = holder
                actual_holder = task_id
            else:
