...
Provides automatic cleanup of temporary files created by OctopusOS to prevent
...
Usage:
    from octopusos.core.maintenance import cleanup_old_temp_files
from octopusos.core.time import utc_now, utc_now_iso
    ...
    from octopusos.core.maintenance import schedule_cleanup_task
...
def get_temp_dirs() -> List[Path]:
    ...
    # OctopusOS-specific temp directories
    octopusos_temp = system_temp / "octopusos"
    if octopusos_temp.exists():
        temp_dirs.append(octopusos_temp)

    # Runtime temp directory
    try:
        runtime_temp = Path.home() / ".octopusos" / "runtime" / "temp"
