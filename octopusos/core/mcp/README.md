...
This module provides integration with Model Context Protocol (MCP) servers, allowing OctopusOS to use tools from MCP-compliant servers alongside native Extensions.
...
```python
from octopusos.core.mcp import MCPConfigManager

manager = MCPConfigManager()  # Loads from ~/.octopusos/mcp_servers.yaml
...
```python
from octopusos.core.mcp import MCPClient, MCPServerConfig
...
```python
from octopusos.core.mcp import MCPHealthChecker, HealthStatus
...
```python
from octopusos.core.capabilities.router import ToolRouter
...
```python
from octopusos.core.mcp import MCPConnectionError
...
```python
from octopusos.core.mcp import MCPTimeoutError
...
```python
from octopusos.core.mcp import MCPProtocolError
...
# Run with coverage
pytest tests/core/mcp/ --cov=octopusos.core.mcp --cov-report=html
```
...
// Initialize
{
  ...
  "params": {
    ...
    "capabilities": {},
    "clientInfo": {"name": "octopusos", "version": "0.3.1"}
  }
}
...
- [OctopusOS Capability Abstraction](../capabilities/README_CAPABILITY_ABSTRACTION.md)
