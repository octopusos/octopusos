    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Runtime compatibility fallback for legacy AWS MCP launcher command.
                package_id = (self.config.env or {}).get("OCTOPUSOS_MCP_PACKAGE_ID")
                is_legacy_aws_launcher = (
                    package_id == "aws.mcp"
                    and bool(self.config.command)
                    and self.config.command[0] == "aws-mcp"
                )
                uvx_path = shutil.which("uvx")
                if not is_legacy_aws_launcher or not uvx_path:
                    raise
                fallback_command = ["uvx", "awslabs.aws-api-mcp-server@latest", *self.config.command[1:]]
                logger.warning(
                    "MCP launcher 'aws-mcp' not found; fallback to '%s' for server %s",
            ...
            try:
                handshake_timeout_ms = max(60000, int(self.config.timeout_ms or 30000))
