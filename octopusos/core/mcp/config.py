...
DEFAULT_CONFIG_PATH = Path.home() / ".octopusos" / "mcp_servers.yaml"
...
class MCPConfigManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        Args:
            config_path: Path to configuration file (default: ~/.octopusos/mcp_servers.yaml)
        """
    ...
    def get_server(self, server_id: str) -> Optional[MCPServerConfig]:
        """Alias of get_server_config for API service call sites."""
        return self.get_server_config(server_id)
    ...
    def list_servers(self) -> List[MCPServerConfig]:
        """List all known server configurations."""
        return self.load_config()

    def add_server(self, server_config: MCPServerConfig) -> MCPServerConfig:
        """Add a new MCP server and persist to disk."""
        if server_config.id in self._configs:
            raise ValueError(f"MCP server already exists: {server_config.id}")
        self._configs[server_config.id] = server_config
        self._save_config()
        return server_config

    def update_server(self, server_id: str, patch: Dict[str, object]) -> MCPServerConfig:
        """Update a server configuration and persist to disk."""
        ...
        updated = MCPServerConfig(**merged)
        self._configs[server_id] = updated
        self._save_config()
        return updated

    def remove_server(self, server_id: str) -> bool:
        """Remove a server configuration and persist to disk."""
    ...
    def _save_config(self) -> None:
        """Persist in-memory config to YAML file."""
        self.config_path.parent.mkdir(parents=True, exist_ok=True)
        payload = {
            "mcp_servers": [cfg.model_dump() for cfg in self._configs.values()]
        }
