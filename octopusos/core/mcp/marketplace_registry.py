...
from octopusos.core.mcp.marketplace_models import (
    MCPPackage,
    MCPGovernancePreview,
    MCPTransportType
)
from octopusos.core.capabilities.trust_tier_defaults import TRUST_TIER_QUOTA_DEFAULTS
...
class MCPMarketplaceRegistry:
    ...
    def _load_registry(self):
        ...
        try:
            try:
                self._loaded_mtime_ns = os.stat(self.registry_path).st_mtime_ns
            except Exception:
    ...
    def maybe_reload(self) -> bool:
        """Reload registry if the YAML file changed on disk.

        This avoids needing a backend restart during local development or UI iteration.
        """
        try:
            if not self.registry_path.exists():
                return False
            mtime_ns = os.stat(self.registry_path).st_mtime_ns
            if self._loaded_mtime_ns is not None and mtime_ns == self._loaded_mtime_ns:
                return False
        except Exception:
            # If we can't stat, don't reload on every request.
            return False
    ...
    def generate_governance_preview(self, package_id: str) -> Optional[MCPGovernancePreview]:
        ...
        from octopusos.core.capabilities.capability_models import TrustTier
        tier_enum_map = {
            "T0": TrustTier.T0,
            "T1": TrustTier.T1,
            "T2": TrustTier.T2,
