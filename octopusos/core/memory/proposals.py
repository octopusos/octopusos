...
from memoryos.migrator import ensure_memoryos_migrations
from octopusos.core.memory.capabilities import MemoryCapability
from octopusos.core.memory.permission import get_permission_service
...
class MemoryProposalService:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        # Allow explicit DB path overrides only in test runs (or when explicitly enabled),
        # while keeping the default hard boundary for production deployments.
        allow_override = bool(os.environ.get("PYTEST_CURRENT_TEST")) or os.environ.get("OCTOPUSOS_ALLOW_DB_PATH_OVERRIDE") == "1"
        self.db_path = resolve_component_db_path("memoryos", db_path, allow_override=allow_override)
        self.permission_service = get_permission_service()
        ensure_memoryos_migrations()
    ...
    def _audit_proposal_event(
        ...
        **kwargs
        ...
        """
        try:
            from octopusos.core.audit import emit_audit_event
