...
The Mode Policy system is a **configuration-driven permission management framework** that controls what operations each execution mode can perform in OctopusOS. It separates permission logic from code, making it easy to customize, audit, and maintain operational constraints.
...
```python
from octopusos.core.mode.mode_policy import load_policy_from_file
...
```python
from octopusos.core.mode.mode_policy import ModePolicy, set_global_policy
from pathlib import Path
...
# Verify it's active
from octopusos.core.mode.mode_policy import get_global_policy
...
Set a policy file path via environment variable before starting OctopusOS.

```bash
# Set policy path
export MODE_POLICY_PATH=/path/to/custom_policy.json

# Run OctopusOS (will load policy at startup)
octopusos run "your command"
```
...
```python
from octopusos.core.mode.mode_policy import check_mode_permission, get_mode_permissions
...
OctopusOS ships with three pre-configured policy files in `configs/mode/`:
...
```python
from pathlib import Path
from octopusos.core.mode.mode_policy import load_policy_from_file, get_global_policy
...
3. Use schema validator (future):
   ```bash
   jsonschema -i my_policy.json octopusos/core/mode/mode_policy.schema.json
   ```
...
from octopusos.core.mode.mode_policy import get_mode_permissions
...
See `octopusos/core/mode/mode_policy.schema.json` for the formal JSON Schema definition.
...
from octopusos.core.mode.mode_policy import check_mode_permission
...
1. Check logs: `tail -f logs/octopusos.log | grep "Policy"`
2. Run verification: `python test_mode_policy_verification.py`
3. Review code: `octopusos/core/mode/mode_policy.py`
