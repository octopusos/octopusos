...
from octopusos.core.model import ModelRegistry, InvocationConfig, ModelCredentialsError
from octopusos.config.cli_settings import load_settings
...
class ModelInvoker:
    ...
    def invoke_api(
        ...
        **kwargs
        ...
        try:
            if brand == "Ollama":
                from octopusos.ext.tools.ollama_adapter import OllamaAdapter
                adapter = OllamaAdapter(model_id)
            elif brand == "LMStudio":
                from octopusos.ext.tools.lmstudio_adapter import LMStudioAdapter
                adapter = LMStudioAdapter(model_id)
            elif brand == "OpenAI":
                from octopusos.ext.tools.openai_chat_adapter import OpenAIChatAdapter
