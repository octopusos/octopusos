...
from octopusos.schemas.v31_models import Repo
from octopusos.core.project.errors import (
    RepoNotFoundError,
...
from octopusos.core.project.path_utils import (
...
class RepoService:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        if self.db_path:
            self._writer = SQLiteWriter(str(self.db_path))
        else:
    ...
    def _get_repo_columns(self, cursor: sqlite3.Cursor) -> set:
        return {row[1] for row in cursor.execute("PRAGMA table_info(repos)")}
    ...
    def add_repo(
        ...
        def _write_repo(conn):
            cursor = conn.cursor()
            columns = self._get_repo_columns(cursor)
            ...
            insert_columns = ["repo_id", "project_id", "name", "local_path", "created_at", "updated_at"]
            insert_values = [repo_id, project_id, name, local_path, now, now]

            if "vcs_type" in columns:
                insert_columns.insert(4, "vcs_type")
                insert_values.insert(4, vcs_type)

            if "remote_url" in columns:
                insert_columns.insert(5, "remote_url")
                insert_values.insert(5, remote_url)

            if "default_branch" in columns:
                insert_columns.insert(6, "default_branch")
                insert_values.insert(6, default_branch)

            placeholders = ", ".join(["?"] * len(insert_columns))
            column_list = ", ".join(insert_columns)
            cursor.execute(
                f"INSERT INTO repos ({column_list}) VALUES ({placeholders})",
                tuple(insert_values),
            )
