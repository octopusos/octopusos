...
class ProjectRepository:
    ...
    def __init__(self, db_path: Path):
        ...
        self._repo_table_name: Optional[str] = None
    ...
    def _get_repo_table(self, conn: sqlite3.Connection) -> str:
        """Determine repo table name (repos vs project_repos)."""
        if self._repo_table_name:
            return self._repo_table_name

        cursor = conn.cursor()
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('repos', 'project_repos')"
        )
        names = {row[0] for row in cursor.fetchall()}
        if "repos" in names:
            self._repo_table_name = "repos"
        elif "project_repos" in names:
            self._repo_table_name = "project_repos"
        else:
            # Default to repos; actual operation will raise a clearer DB error
            self._repo_table_name = "repos"
    ...
    def add_repo(self, repo_spec: RepoSpec) -> str:
        ...
        try:
            cursor = conn.cursor()
            table = self._get_repo_table(conn)
