...
from octopusos.schemas.v31_models import Project, Repo
from octopusos.core.project.errors import (
    ProjectNotFoundError,
    ProjectNameConflictError,
    ProjectHasTasksError,
)
from octopusos.core.task.models import Task
...
class ProjectService:
    ...
    def _get_project_columns(self, cursor: sqlite3.Cursor) -> set:
        return {row[1] for row in cursor.execute("PRAGMA table_info(projects)")}
    ...
    def create_project(
        ...
        now = utc_now_iso()
        project_path = ensure_project_path(project_id, name=name)
        ...
        def _write_project(conn):
            cursor = conn.cursor()
            columns = self._get_project_columns(cursor)
            ...
            if "path" in columns:
                cursor.execute(
                    """
                    INSERT INTO projects (project_id, name, description, tags, default_repo_id, created_at, updated_at, path)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        project_id,
    ...
    def get_project(self, project_id: str) -> Optional[Project]:
        ...
        try:
            ...
            cursor.execute(
                ...
                {', path' if 'path' in self._get_project_columns(cursor) else ''}
                FROM projects
                WHERE project_id = ?
                """,
                (project_id,),
    ...
    def list_projects(
        ...
        try:
            ...
            if tags:
                ...
                columns = self._get_project_columns(cursor)
                path_select = ", path" if "path" in columns else ""
                query = f"""
                    SELECT project_id, name, description, tags, default_repo_id, created_at, updated_at, metadata{path_select}
                    FROM projects
                    WHERE 1=1
                """
