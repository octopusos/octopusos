...
from octopusos.core.task import TaskManager, Task, RunMode
from octopusos.core.task.run_mode import TaskMetadata
from octopusos.core.task.project_settings_inheritance import ProjectSettingsInheritance
...
from octopusos.core.gates.done_gate import DoneGateRunner, GateRunResult
from octopusos.core.mode.pipeline_runner import ModePipelineRunner
from octopusos.core.mode.mode_selector import ModeSelection
from octopusos.router import Router, RoutePlan, RerouteReason
...
from octopusos.core.checkpoints import CheckpointManager, Evidence, EvidencePack, EvidenceType
from octopusos.core.worker_pool import LeaseManager, start_heartbeat
from octopusos.core.idempotency import LLMOutputCache, ToolLedger
from octopusos.store import get_db
from octopusos.core.task.event_service import (
    emit_runner_spawn,
    emit_phase_enter,
    emit_phase_exit,
...
class TaskRunner:
    ...
    def __init__(
        ...
        self.action_kb = ActionableKBService()
    ...
    def run_task(self, task_id: str, max_iterations: int = 100):
        ...
        """
        import os
        from octopusos.core.task.timeout_manager import TimeoutManager
        from octopusos.core.task.cancel_handler import CancelHandler
        ...
        try:
            ...
            try:
                self._ensure_failure_summary_and_brain_decision(
                    task_id=task_id,
                    exit_reason=exit_reason,
                )
            except Exception as e:
                logger.error(f"Failed to finalize failure summary/brain decision: {e}", exc_info=True)
            ...
            try:
                from octopusos.core.task.event_service import TaskEventService
        ...
        if current_status == "created":
            ...
            try:
                ...
                if gate_results.all_passed:
                    kb_validation = self.action_kb.validate_task_success(task.task_id)
                    required_types = task.metadata.get("required_evidence_types")
                    ...
                    evidence_validation = EvidenceGate.validate(
                        required_types=[str(t) for t in required_types],
                        evidence_refs=evidence_refs,
                        must_exist_on_disk=True,
                    )

                    if not kb_validation.ok or not evidence_validation.ok:
                        task.metadata["success_gate"] = {
                            "result": "FAILED",
                            "kb_validation": kb_validation.to_dict(),
                        ...
                        self._log_audit(
                            task.task_id,
                            "error",
                            f"SUCCESS_GATE_FAILED: kb_ok={kb_validation.ok}, evidence_ok={evidence_validation.ok}",
                        )
                        return "failed"
    ...
    def _ensure_failure_summary_and_brain_decision(self, *, task_id: str, exit_reason: str) -> None:
        task = self.task_manager.get_task(task_id)
        if not task:
            return
        ...
        if not isinstance(failure_summary, dict):
            ...
            summary_obj = build_failure_summary(
                exit_reason=exit_reason,
                reason=f"Task ended with status={status}, exit_reason={exit_reason}",
                evidence_refs=evidence_refs,
            )
            failure_summary = summary_obj.to_dict()
            metadata["failure_summary"] = failure_summary
            self._update_task_metadata(task_id, metadata)
            self.task_manager.add_audit(
                task_id=task_id,
                event_type="FAILURE_SUMMARY",
                level="error",
                payload=failure_summary,
            )

        decision = consume_failure_event(task_id=task_id, failure_summary=failure_summary)
        self.task_manager.add_audit(
            task_id=task_id,
            event_type="BRAIN_FAILURE_DECISION",
            level="info",
            payload=decision,
        )
