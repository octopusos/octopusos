...
```python
from octopusos.core.task.dependency_service import TaskDependencyService
from octopusos.core.task.runner_integration import prepare_execution_env
...
**Trigger**: Task reads `.octopusos/artifacts/<task-id>.json`

**Example**:
```python
# Task B reads artifact file for Task A
artifact_path = Path(".octopusos/artifacts/task-a.json")
...
```bash
# Create dependency
octopusos task dependencies create task-002 task-001 \
  --type requires \
...
- **Schema**: `/octopusos/store/migrations/v18_multi_repo_projects.sql`
- **Models**: `/octopusos/core/task/models.py` (TaskDependency, DependencyType)
- **Artifact Service**: `/octopusos/core/task/artifact_service.py`
- **Audit Service**: `/octopusos/core/task/audit_service.py`
- **Runner Integration**: `/octopusos/core/task/runner_integration.py`
