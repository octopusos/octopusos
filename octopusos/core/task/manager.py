...
class TaskManager:
    ...
    def create_task(
        ...
        try:
            ...
            if auto_created_session:
                cursor.execute(
                    "UPDATE chat_sessions SET task_id = ? WHERE session_id = ?",
                    (task_id, session_id),
                )
            conn.commit()
    ...
    def update_task_status(self, task_id: str, status: str) -> None:
        ...
        warnings.warn(
            ...
            "See octopusos/core/task/MIGRATION_GUIDE.md for details.",
            DeprecationWarning,
            stacklevel=2
        )
    ...
    def require_kb_execution(self, task_id: str, kb_id: str) -> None:
        """Require Action KB execution before task can be marked successful."""
        from octopusos.core.growth.actionable_kb import ActionableKBService

        service = ActionableKBService()
        service.require_kb_execution(task_id=task_id, kb_id=kb_id)
        self.add_audit(
            task_id=task_id,
            event_type="TASK_KB_REQUIRED",
            level="info",
            payload={"kb_id": kb_id},
        )

    def record_kb_execution(
        self,
        *,
        task_id: str,
        kb_id: str,
        ...
        """Record Action KB execution evidence for a task."""
        from octopusos.core.growth.actionable_kb import ActionableKBService
        ...
        self.add_audit(
            task_id=task_id,
            event_type="TASK_KB_EXECUTED",
            level="info" if verify_passed else "warn",
            payload={
                "kb_id": kb_id,
                "verify_passed": verify_passed,
                "evidence_refs": evidence_refs or [],
