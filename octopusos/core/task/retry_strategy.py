...
Distinct from tool-level retry (octopusos/ext/tools/retry_policy.py).
...
class RetryStrategyManager:
    ...
    def calculate_next_retry_time(
        ...
        if retry_config.backoff_type == RetryBackoffType.NONE:
            ...
            # For exponential backoff, use 2^(retry_count) so first retry has base_delay
            # retry_count=0 -> 2^0 = 1 -> base_delay
            # retry_count=1 -> 2^1 = 2 -> base_delay * 2
            # retry_count=2 -> 2^2 = 4 -> base_delay * 4
