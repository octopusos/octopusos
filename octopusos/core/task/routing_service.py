...
from octopusos.router import Router, RoutePlan
from octopusos.core.task.manager import TaskManager
from octopusos.core.growth.skill_learning import SkillLearningService
...
class TaskRoutingService:
    ...
    async def route_new_task(
        ...
        try:
            ...
            # Apply approved routing hint if available.
            hint = self.skill_learning.get_active_hint("task_routing")
            if hint:
                preferred_instance = str(hint.get("preferred_params", {}).get("preferred_instance") or "").strip()
                if preferred_instance and preferred_instance in plan.scores:
                    if plan.selected != preferred_instance:
                        previous = plan.selected
                        plan.selected = preferred_instance
                        if previous not in plan.fallback:
                            plan.fallback.insert(0, previous)
                        plan.reasons.append(
                            self.skill_learning.explain_hint_usage("task_routing", hint)
                        )
                        self.task_manager.add_audit(
                            task_id=task_id,
                            event_type="SKILL_HINT_APPLIED",
                            level="info",
                            payload={
                                "skill_id": "task_routing",
                                "preferred_instance": preferred_instance,
                                "previous_instance": previous,
                                "sample_size": hint.get("sample_size", 0),
            ...
            # Learn routing behavior after every routed task.
            self.skill_learning.record_execution(
                skill_id="task_routing",
                params={"preferred_instance": plan.selected},
                status="success",
                latency_ms=None,
                error_code=None,
                evidence_refs=[{"type": "trace", "path": f"task://{task_id}/route"}],
            )
            self.skill_learning.generate_behavior_hint("task_routing", min_samples=3)
