...
from octopusos.core.task.models import Task, TaskLineageEntry, TaskTrace
from octopusos.core.task.states import TaskState, is_valid_state
...
from octopusos.store import get_db, get_writer
...
class TaskService:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        try:
            if db_path is None:
                ...
                init_db(auto_migrate_after_init=True)
    ...
    def create_draft_task(
        ...
        def _write_task_to_db(conn):
            ...
            session_inserted = auto_created_session
            if session_inserted and session_id:
                try:
                    cursor.execute(
                        """
                    ...
                    if "no such table: chat_sessions" in str(e):
                        logger.warning("chat_sessions table missing; skipping auto session insert")
            ...
            if session_inserted and session_id:
                try:
                    ...
                    if "no such table: chat_sessions" in str(e):
                        logger.warning("chat_sessions table missing; skipping session link")
                    else:
                        raise
        ...
        try:
            ...
            if self.db_path is None and "no such table" in str(e):
                try:
                    from octopusos.store import init_db
                    init_db(auto_migrate_after_init=True)
                    from octopusos.core.db import SQLiteWriter
                    from octopusos.core.storage.paths import component_db_path
                    fresh_writer = SQLiteWriter(str(component_db_path("octopusos")))
                    result_task_id = fresh_writer.submit(_write_task_to_db, timeout=10.0)
                    logger.info(f"Created draft task after init: {result_task_id} (session: {session_id})")
                except Exception:
        ...
        try:
            from octopusos.core.task.routing_service import TaskRoutingService
            import asyncio
    ...
    def fail_task(
        ...
        if metadata is None:
            metadata = {}
        if "exit_reason" not in metadata:
            metadata["exit_reason"] = "unknown"
        return self.state_machine.transition(
            task_id=task_id,
    ...
    def retry_failed_task(
        ...
        Raises:
            ...
            TaskNotFoundError: If task doesn't exist
        """
        from octopusos.core.task.retry_strategy import RetryStrategyManager
        from octopusos.core.task.errors import RetryNotAllowedError
    ...
    def get_guardian_reviews(self, task_id: str) -> List[Any]:
        ...
        """
        try:
            from octopusos.core.guardian import GuardianService
