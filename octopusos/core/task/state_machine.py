...
from octopusos.core.task.states import TaskState, ALL_STATES, TERMINAL_STATES
...
class TaskStateMachine:
        ...
        try:
            ...
            if self.db_path is None and "no such table" in str(e):
                try:
                    ...
                    writer = SQLiteWriter(str(component_db_path("octopusos")))
                    writer.submit(_execute_transition, timeout=10.0)
                except Exception as retry_error:
                    logger.error(f"Error during state transition for task {task_id}: {str(retry_error)}")
                    raise TaskStateError(
                        f"Failed to transition task: {str(retry_error)}",
                        task_id=task_id
                    )
            else:
    ...
    def _get_mode_gateway(self, mode_id: str):
        ...
        """
        try:
            from octopusos.core.mode.gateway_registry import get_mode_gateway
            ...
            from octopusos.core.mode.gateway_registry import DefaultModeGateway
