...
class WorkspaceRoot:
    ...
    def __post_init__(self):
        ...
        # Normalize path without resolving symlinks (keeps /tmp vs /private/tmp)
        normalized = Path(self.root_path).expanduser().absolute()
        self.root_path = Path(os.path.normpath(str(normalized)))
...
class WorkspaceLayout:
    ...
    - Metadata: <project_root>/.octopusos/
    """
    ...
    def get_repo_path(self, project_id: str, repo: RepoSpec) -> Path:
        ...
        # Normalize path without resolving symlinks (keeps /tmp vs /private/tmp)
        repo_path = Path(os.path.normpath(str(repo_path.absolute())))

        return repo_path

    def get_metadata_dir(self, project_id: str) -> Path:
        """Get OctopusOS metadata directory for a project

        Args:
            project_id: Project identifier

        Returns:
            Path to .octopusos/ metadata directory

        Example:
            /workspace/projects/my-app/.octopusos/
        """
        return self.get_project_root(project_id) / ".octopusos"
    ...
    def validate_layout(self, project_id: str, repos: List[RepoSpec]) -> "ValidationResult":
        ...
        """
        from octopusos.core.workspace.validation import ValidationResult, Conflict, ConflictType
