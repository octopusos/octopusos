...
# 导入跨平台文件锁工具
from octopusos.core.utils.filelock import acquire_lock, release_lock, LockAcquisitionError

# 导入 Lead Agent 组件
from octopusos.core.lead.adapters.storage import LeadStorage
from octopusos.core.lead.adapters.task_creator import LeadTaskCreator
from octopusos.core.lead.contract import ContractMapper
from octopusos.core.lead.dedupe import LeadFindingStore
from octopusos.core.lead.dedupe import LeadFinding as DedupeLeadFinding
from octopusos.core.lead.miner import RiskMiner, MinerConfig
from octopusos.core.lead.models import ScanWindow, WindowKind
from octopusos.core.lead.models import LeadFinding as MinerLeadFinding

# 导入配置管理
from octopusos.config import load_lead_config, LeadConfig
...
# 锁文件路径 (跨平台临时目录)
LOCK_FILE_PATH = Path(tempfile.gettempdir()) / "octopusos_lead_scan.lock"
...
class LeadScanJob:
    ...
    def __init__(
        ...
        Args:
            db_path: 数据库路径（默认使用环境变量 OCTOPUSOS_LEAD_SCAN_DB 或 ~/.octopusos/store.db）
        ...
        if db_path is None:
            ...
            db_path_str = os.getenv("OCTOPUSOS_LEAD_SCAN_DB")
            if db_path_str:
    ...
    def _check_contract_versions(self, dry_run: bool = True) -> dict:
        ...
        """
        from octopusos.core.lead.adapters.storage import LeadStorage
        from octopusos.core.lead.miner import RiskMiner
...
"""
    ...
    parser.add_argument(
        "--db-path",
        type=Path,
        help="数据库路径（默认: ~/.octopusos/store/octopusos/db.sqlite）"
    )
