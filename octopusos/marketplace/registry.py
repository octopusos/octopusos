...
from octopusos.marketplace.manifest import (
    CapabilityManifest,
    load_manifest,
    normalize_manifest,
    validate_manifest,
...
class MarketplaceRegistry:
    ...
    def __init__(self, db_path: Optional[Union[str, Path]] = None):
        ...
        Args:
            db_path: Path to SQLite database. If None, uses default OctopusOS database.
        """
        if db_path is None:
        ...
        if not self.db_path.exists():
            raise RegistryError(f"Database not found: {self.db_path}. Run 'octopusos init' first.")
