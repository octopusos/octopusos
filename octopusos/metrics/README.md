...
from octopusos.metrics.info_need_metrics import InfoNeedMetrics
...
python -m octopusos.cli.metrics show

# Show metrics for last 7 days
python -m octopusos.cli.metrics show --last 7d

# Generate report and save to file
python -m octopusos.cli.metrics generate --output report.json

# Generate report for specific date range
python -m octopusos.cli.metrics generate \
    --start "2025-01-01" \
    --end "2025-01-31" \
    --output jan_report.json

# Export metrics as CSV
python -m octopusos.cli.metrics export --last 24h --format csv --output metrics.csv
```
...
# Run with coverage
pytest tests/unit/metrics/ --cov=octopusos.metrics --cov-report=html
```
...
```bash
# Add to crontab
0 2 * * * cd /path/to/octopusos && python -m octopusos.cli.metrics generate --last 24h --output /var/log/metrics/daily_$(date +\%Y\%m\%d).json
```

### Systemd Timer Example

Create `/etc/systemd/system/octopusos-metrics.service`:

```ini
[Unit]
Description=OctopusOS InfoNeed Metrics Report

[Service]
Type=oneshot
ExecStart=/usr/bin/python -m octopusos.cli.metrics generate --last 24h --output /var/log/metrics/daily_report.json
WorkingDirectory=/opt/octopusos
User=octopusos
```

Create `/etc/systemd/system/octopusos-metrics.timer`:

```ini
[Unit]
Description=Daily OctopusOS Metrics
...
```bash
systemctl enable octopusos-metrics.timer
systemctl start octopusos-metrics.timer
```
