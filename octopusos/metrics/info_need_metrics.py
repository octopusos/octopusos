...
class InfoNeedMetrics:
    ...
    def _load_classifications(
        ...
        try:
            ...
            """, (INFO_NEED_CLASSIFICATION, INFO_NEED_CLASSIFICATION.lower()))
            ...
            for row in rows:
                ...
                parsed = parse_db_time(created_at)
                if parsed is None and isinstance(created_at, (int, float)):
                    parsed = datetime.fromtimestamp(created_at, tz=timezone.utc)
                if parsed is None and isinstance(created_at, str) and created_at.isdigit():
                    parsed = datetime.fromtimestamp(int(created_at), tz=timezone.utc)
                if parsed is None:
                    continue
    ...
    def _load_outcomes(
        ...
        try:
            cursor.execute("""
                ...
                WHERE event_type IN (?, ?)
                ORDER BY created_at ASC
            """, (INFO_NEED_OUTCOME, INFO_NEED_OUTCOME.lower()))
