"""
Cross-platform utility module for OctopusOS providers.
...
def get_config_dir() -> Path:
    """
    Get the OctopusOS configuration directory for the current platform.

    Returns:
        Path: Configuration directory path
            - Windows: %APPDATA%\\octopusos (e.g., C:\\Users\\User\\AppData\\Roaming\\octopusos)
            - macOS/Linux: ~/.octopusos

    Examples:
        >>> get_config_dir()
        Path('/Users/username/.octopusos')  # macOS
        Path('C:/Users/username/AppData/Roaming/octopusos')  # Windows
    """
    if get_platform() == 'windows':
        # Windows: Use AppData/Roaming directory
        appdata = os.environ.get('APPDATA')
        if appdata:
            return Path(appdata) / 'octopusos'
        else:
            # Fallback if APPDATA is not set
            return Path.home() / 'AppData' / 'Roaming' / 'octopusos'
    else:
...
def get_run_dir() -> Path:
    ...
    Examples:
        >>> get_run_dir()
        Path('/Users/username/.octopusos/run')
    """
...
def get_log_dir() -> Path:
    ...
    Examples:
        >>> get_log_dir()
        Path('/Users/username/.octopusos/logs')
    """
...
def find_executable(name: str, custom_paths: Optional[list[str]] = None) -> Optional[Path]:
    ...
    # If explicit custom paths are provided and none are valid, do not fall back.
...
def get_models_dir(provider_name: str) -> Optional[Path]:
    ...
    if provider_name == 'ollama':
        ...
        # Provide a sensible default per platform for local models
        if platform_type == 'windows':
            return Path.home() / 'Documents' / 'AI Models'
        elif platform_type == 'linux':
            # Avoid spaces for common Linux tooling
            return Path.home() / 'AI_Models'
        else:
            # macOS default
            return Path.home() / 'Documents' / 'AI Models'
