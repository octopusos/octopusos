...
from octopusos.providers.platform_utils import get_run_dir, get_log_dir
from octopusos.core.utils.process import terminate_process, kill_process
...
async def check_port_in_use(host: str, port: int, timeout: float = 0.5) -> bool:
...
class ProcessManager:
    ...
    async def start_process(
        ...
        # Check port availability (only when args provided)
        if check_port and args:
        ...
        try:
            if command is None:
                if bin_name is None or args is None:
                    return False, "Missing command or bin_name/args"
            ...
            self._start_output_readers(instance_key)
    ...
    async def stop_process(
        ...
        Uses terminate_process() and kill_process() from octopusos.core.utils.process,
    ...
    def _start_output_readers(self, instance_key: str) -> None:
        """Start background output readers for a process."""
        asyncio.create_task(self._monitor_process(instance_key))

    def _stop_output_readers(self, instance_key: str) -> None:
        """Stop background output readers for a process."""
        tasks = self._reader_tasks.get(instance_key)
        if not tasks:
            return
        for task in tasks:
            if not task.done():
    ...
    def list_processes(self) -> Dict[str, Dict[str, Any]]:
        """Compatibility wrapper for list_all_processes()."""
        return self.list_all_processes()
