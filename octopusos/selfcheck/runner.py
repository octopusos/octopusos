...
Provides a single entry point for diagnosing OctopusOS health:
- Runtime environment
...
class SelfCheckRunner:
    """
    Self-check runner for OctopusOS
    ...
    def _get_version(self) -> str:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return f"octopusos 0.3.2 (git {sha})"
        except Exception:
            pass

        return "octopusos 0.3.2"

    def _check_paths(self) -> CheckItem:
        """Check critical paths are writable"""
        home = Path.home()
        paths = {
            "runtime": home / ".octopusos" / "runtime",
            "secrets": home / ".octopusos" / "secrets",
        }
        ...
        if not issues:
            return CheckItem(
                ...
                status="PASS",
                detail=f"~/.octopusos/runtime, ~/.octopusos/secrets",
                hint=None,
                actions=[],
            )
        else:
            return CheckItem(
                ...
                status="FAIL",
                detail="; ".join(issues),
                hint="Check filesystem permissions for ~/.octopusos",
                actions=[],
            )

    def _check_permissions(self) -> CheckItem:
        """Check secrets file permissions"""
        secrets_file = Path.home() / ".octopusos" / "secrets" / "providers.json"
        ...
        if mode == 0o600:
            ...
            return CheckItem(
                ...
                hint="Run: chmod 600 ~/.octopusos/secrets/providers.json",
                actions=[],
            )
    ...
    async def _check_providers(self, include_network: bool = False, target_provider: Optional[str] = None) -> List[CheckItem]:
        ...
        from octopusos.providers.base import ProviderStatus, ProviderState
    ...
    async def _check_memory_store(self) -> CheckItem:
        ...
        try:
            ...
            home = Path.home()
            memory_path = home / ".octopusos" / "memory"
            ...
            if content == "test":
                ...
                return CheckItem(
                    ...
                    hint="Check ~/.octopusos/memory permissions",
                    actions=[],
                )
            ...
            return CheckItem(
                ...
                hint="Check ~/.octopusos/memory directory permissions",
                actions=[],
            )
