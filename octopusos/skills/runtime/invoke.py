...
- Reuses octopusos.core.chat.guards.phase_gate.PhaseGate
- Follows same phase naming (planning/execution)
- Compatible with session-based phase management

Usage:
    >>> from octopusos.skills.runtime import SkillLoader, SkillInvoker
...
from .dbops_bridge import DBOpsBridge
...
class SkillInvoker:
    ...
    def __init__(self, loader: SkillLoader, execution_phase: str = 'planning'):
        ...
        self._dbops_bridge: DBOpsBridge | None = None
    ...
    def invoke(self, skill_id: str, command: str, args: Dict[str, Any]) -> Any:
        ...
        # DB Ops bridge: route db skill actions into controlled orchestration path.
        # This branch is evaluated before generic skill registry checks to allow
        # db_ops to operate as a governed system capability.
        if self._is_dbops_action(skill_id, command, args):
            bridge = self._get_dbops_bridge(args)
            request_id = str(args.get("request_id") or args.get("session_id") or "")
            actor = str(args.get("requested_by") or args.get("user_id") or "skill_invoker")
    ...
    def _execute_skill(self, skill: Dict[str, Any], command: str, args: Dict[str, Any]) -> Any:
        ...
        Security Limitations (MVP):
            ...
            - Skills run in same process as OctopusOS
        ...
        # Format: ~/.octopusos/store/skills_cache/{skill_id}/{repo_hash}/
        cache_dir = Path.home() / ".octopusos" / "store" / "skills_cache" / skill['skill_id']
        ...
        # Security Note: This runs in the same process as OctopusOS
    ...
    def _is_dbops_action(self, skill_id: str, command: str, args: Dict[str, Any]) -> bool:
        action_id = str(args.get("action_id") or command or "")
        return skill_id == "db_ops" or action_id.startswith("db.") or command.startswith("db.")

    def _get_dbops_bridge(self, args: Dict[str, Any]) -> DBOpsBridge:
        catalog_ref = args.get("provider_catalog_ref")
        idem_ref = args.get("idempotency_store_ref") or os.getenv("OCTOPUSOS_DBOPS_IDEMPOTENCY_DB")
        if catalog_ref:
            return DBOpsBridge(catalog_ref, idempotency_db_path=idem_ref)

        if self._dbops_bridge is None:
            default_ref = os.getenv("OCTOPUSOS_DBOPS_CATALOG", "docs/ops/db_provider_catalog.example.json")
            self._dbops_bridge = DBOpsBridge(default_ref, idempotency_db_path=idem_ref)
        return self._dbops_bridge
