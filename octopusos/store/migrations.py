"""Database migration utilities for OctopusOS Store
...
class MigrationManager:
    """Legacy migration manager for tests requiring v0.2 schema."""

    def __init__(self, db_path: Path | str):
        self.db_path = Path(db_path)
        self.migrations_dir = Path(__file__).parent / "migrations"

    def migrate_to_v02(self) -> None:
        """Apply schema_v01.sql and schema_v02.sql to the database."""
        self._apply_sql_file(self.migrations_dir / "schema_v01.sql")
        self._apply_sql_file(self.migrations_dir / "schema_v02.sql")
...
║ 常见解决方案:
║  1. 检查数据库文件权限
║  2. 查看完整日志: octopusos migrate --verbose
║  3. 备份数据库文件
║  4. 寻求帮助: github.com/octopusos/issues
╚══════════════════════════════════════════════════════════════════
"""
...
def _normalize_version(version: str) -> Optional[str]:
    ...
    match = re.fullmatch(r'0\.(\d+)\.0', version)
    if match:
        return version
    match = re.fullmatch(r'v(\d+)', version)
    if match:
        return f"0.{int(match.group(1))}.0"
    return None
...
def get_current_version(conn: sqlite3.Connection) -> Optional[str]:
    ...
    for table_name in ("schema_versions", "schema_version"):
        ...
        try:
            ...
            continue
        for row in results:
            normalized = _normalize_version(row[0])
            if normalized:
