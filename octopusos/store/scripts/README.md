...
Maintenance and migration scripts for OctopusOS storage layer.
...
**Usage**:
```bash
sqlite3 ~/.octopusos/store.db < verify_backfill.sql
```
...
2. OctopusOS database at `~/.octopusos/store.db` (or custom path)
...
```bash
# 1. Backup database
cp ~/.octopusos/store.db ~/.octopusos/store.db.backup
...
```bash
sqlite3 ~/.octopusos/store.db < octopusos/store/migrations/v21_audit_decision_fields.sql
```
...
1. Create script in `octopusos/store/scripts/`
...
**Minimum OctopusOS version**: 0.21.0
