...
示例:
  ...
  python backfill_audit_decision_fields.py --db-path /path/to/store.db
    ...
    parser.add_argument(
        "--db-path",
        type=Path,
        default=Path.home() / ".octopusos" / "store.db",
        help="数据库路径（默认: ~/.octopusos/store.db）"
    )
    ...
    if "source_event_ts" not in columns or "supervisor_processed_at" not in columns:
        ...
        print("请先执行: sqlite3 ~/.octopusos/store.db < octopusos/store/migrations/v21_audit_decision_fields.sql")
        conn.close()
        sys.exit(1)
