...
The SMS Channel enables one-way SMS notification delivery from OctopusOS to mobile phones. Version 1 is **send-only** - it sends SMS messages but does not process replies. This is ideal for alerts, notifications, status updates, and one-way communications.
...
```python
from octopusos.communicationos.channels.sms import SmsAdapter
...
See `octopusos/communicationos/providers/sms/__init__.py` for full protocol definition.
...
5. **Configure in OctopusOS**: Enter credentials
