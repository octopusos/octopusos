...
class CommandProcessor:
    ...
    def __init__(self, session_store=None):
        ...
        if session_store is None:
            from octopusos.communicationos.session_store import SessionStore as PersistentSessionStore
    ...
    def process_command(
        ...
        if command == "/session":
            ...
            # Shortcut aliases for quick session reset in IM channels.
            return self._session_new(channel_id, user_key, conversation_key)
        elif command == "/resume":
            # Shortcut alias for restoring previous session in IM channels.
            return self._session_resume(channel_id, user_key, conversation_key)
    ...
    def _handle_session_command(
        ...
        if subcommand == "new":
            ...
            return self._error_response(
                ...
                "Available: new, id, list, resume, use, close"
            )
    ...
    def _session_resume(
        ...
        Switches current binding back to the previous recent active session.
        """
        current = self.session_store.get_active_session(channel_id, user_key, conversation_key)
        current_id = current["session_id"] if current else None
        sessions = self.session_store.list_sessions(channel_id, user_key, limit=50)

        target = None
        for session in sessions:
            sid = session["session_id"]
            status = str(session.get("status", "active")).lower()
            if sid == current_id:
                continue
        ...
        if not target:
            return self._error_response(
                ...
                user_key,
                conversation_key,
                "ℹ️ No previous session available.\n\nCreate one with: /session new"
            )
        ...
        try:
            ...
            response_text = (
                f"✅ Resumed previous session: {target}\n\n"
                "You are now back on your previous context."
            )
        except ValueError as e:
            response_text = f"❌ Failed to resume previous session: {str(e)}"
        ...
        return OutboundMessage(
            ...
            type=MessageType.TEXT,
            text=response_text,
            metadata={"command": "session_resume", "session_id": target}
        )
    ...
    def _session_close(
        ...
        try:
            ...
            response_text = (
                ...
                f"The session has been closed. Create a new session with: /session new"
            )
    ...
    def _handle_help_command(
        ...
        """
        help_text = """🤖 OctopusOS CommunicationOS - Available Commands

📋 Session Management:
  /session new         - Create and activate new session
  /session resume      - Resume previous recent session
