...
class ChannelCapability(str, Enum):
    ...
    INBOUND_VIDEO = "inbound_video"            # Receive video attachments
    OUTBOUND_VIDEO = "outbound_video"          # Send video attachments
    INBOUND_FILE = "inbound_file"              # Receive file attachments
    OUTBOUND_FILE = "outbound_file"            # Send file attachments
    INBOUND_LOCATION = "inbound_location"      # Receive location messages
    OUTBOUND_LOCATION = "outbound_location"    # Send location messages
    INTERACTIVE = "interactive"                 # Support interactive elements (buttons, menus)
    INTERACTIVE_BUTTONS = "interactive_buttons" # Legacy/manifest alias for buttons
    THREADING = "threading"                     # Support conversation threads/rooms
    THREADED_CONVERSATIONS = "threaded_conversations"  # Legacy/manifest alias
    MENTIONS = "mentions"                       # Mentions (@user) support
    RICH_FORMATTING = "rich_formatting"         # Markdown/rich text formatting support
...
class ChannelManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> ChannelManifest:
        ...
        # Parse capabilities (tolerate legacy aliases by extending enum above).
        capabilities = [ChannelCapability(cap) for cap in data.get("capabilities", [])]

        # Parse session scope with compatibility mapping.
        scope_raw = str(data.get("session_scope", "user") or "user").strip().lower()
        if scope_raw in {"thread", "threaded", "conversation", "room"}:
            scope_raw = "user_conversation"
        ...
        return cls(
            ...
            session_scope=SessionScope(scope_raw),
