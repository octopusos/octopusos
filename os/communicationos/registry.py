...
from octopusos.communicationos.manifest import ChannelManifest, SecurityMode
...
class ChannelConfigStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self._init_database()
        self._cipher: Optional["_ConfigSecretsCipher"] = None

    def _get_cipher(self) -> "_ConfigSecretsCipher":
        if self._cipher is None:
            self._cipher = _ConfigSecretsCipher()
        return self._cipher

    def _secret_field_names(self, channel_id: str) -> set[str]:
        try:
            manifest = ChannelRegistry().get_manifest(channel_id)
        except Exception:
            manifest = None
        if not manifest:
            return set()
        return {f.name for f in (manifest.required_config_fields or []) if getattr(f, "secret", False)}

    def _encrypt_secrets(self, channel_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
        secret_fields = self._secret_field_names(channel_id)
    ...
    def _decrypt_secrets(self, channel_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
        secret_fields = self._secret_field_names(channel_id)
        if not secret_fields:
            return dict(config or {})
        out = dict(config or {})
        ...
        for k in secret_fields:
            ...
            token = v[len("enc::"):]
            try:
                out[k] = cipher.decrypt(token)
            except Exception:
                # Corrupted or wrong key; keep masked sentinel.
                out[k] = ""
        return out
    ...
    def save_config(
        ...
        config_enc = self._encrypt_secrets(channel_id, config or {})
        config_json = json.dumps(config_enc)
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if row:
                ...
                if isinstance(raw, dict):
                    return self._decrypt_secrets(channel_id, raw)
                return raw
            return None
...
class _ConfigSecretsCipher:
    """Local-only Fernet key for encrypting channel secrets at rest."""

    def __init__(self):
        from cryptography.fernet import Fernet

        self._Fernet = Fernet
        key_path = os.getenv("OCTOPUSOS_COMMUNICATIONOS_KEY_FILE", "").strip()
        self._key_file = (
            Path(key_path).expanduser()
            if key_path
            else (Path.home() / ".octopusos" / "communicationos.key")
        )
