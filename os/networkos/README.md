...
NetworkOS is OctopusOS's network control plane, managing public tunnels (Cloudflare Tunnel/ngrok/Tailscale) to make local services publicly accessible.
...
# Start tunnel
octopusos networkos start TUNNEL_ID

# View status
octopusos networkos list
octopusos networkos status TUNNEL_ID
```
...
```
Fix: Enable WAL mode
$ sqlite3 ~/.octopusos/store/networkos/db.sqlite 'PRAGMA journal_mode=WAL;'
```

**Issue**: Database is read-only
```
Fix: Check file permissions
$ chmod 644 ~/.octopusos/store/networkos/db.sqlite
```
...
**A**: Run `octopusos networkos logs TUNNEL_ID` to see error details

### Q: How do I verify NetworkOS is working?
**A**: Run `octopusos doctor` - the networkos check should show ✅ or ⚠️ (warning is OK if DB not yet initialized)

## Security Recommendations

- ✅ Store tokens in encrypted storage (don't commit to Git)
- ✅ Only expose necessary ports/paths
- ✅ Regularly check `octopusos networkos list` to confirm running status
- ✅ Use `octopusos doctor` to verify health
...
NetworkOS uses three main tables in the OctopusOS database:
...
Schema migration: `octopusos/store/migrations/schema_v54_networkos.sql`
...
```python
from octopusos.networkos import NetworkOSStore, Tunnel
...
# Result: ~/.octopusos/store/networkos/db.sqlite
```
...
python -c "from octopusos.store.migrator import get_migration_status; \
           from octopusos.core.storage.paths import component_db_path; \
           print(get_migration_status(component_db_path('octopusos')))"
```
...
# Run with coverage
pytest tests/unit/networkos/ --cov=octopusos.networkos --cov-report=term-missing
```
...
1. Follow the OctopusOS Time Contract (epoch_ms for all timestamps)
...
- Migration: `octopusos/store/migrations/schema_v54_networkos.sql`
- Time Contract: `docs/adr/ADR-011-time-timestamp-contract.md`
- Storage Pattern: `octopusos/core/storage/paths.py`
