...
class NetworkOSStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        """
        if db_path is None:
            canonical_path = resolve_component_db_path("networkos", None)
            ensure_db_exists("networkos")
            ensure_component_migrations("networkos")
            self.db_path = str(canonical_path)
        else:
            # Allow explicit overrides for tests and isolated deployments.
            # This does not cross-write other component DBs because the caller
            # intentionally provides the path.
            # Do not resolve symlinks here: tests expect the original path string.
            p = Path(db_path)
            self.db_path = str(p)
