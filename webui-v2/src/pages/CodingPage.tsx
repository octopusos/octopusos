  ...
  return (
    <Paper sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, borderRadius: 2, boxShadow: 2, overflow: 'hidden', bgcolor: 'background.paper' }}>
      <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, flexWrap: 'wrap' }}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
          <Select size="small" value={sessionId} onChange={(event) => setSessionId(String(event.target.value))} sx={{ minWidth: 220 }}>
            {sessions.map((session) => (
              <MenuItem key={session.session_id} value={session.session_id}>{session.title}</MenuItem>
            ))}
          </Select>
          <Button variant="outlined" onClick={() => void loadSessions()}>{t(K.common.refresh)}</Button>
        </Stack>
      </Box>
      ...
      <Box sx={{ px: 2, py: 1.5, display: 'grid', gap: 1.5, gridTemplateColumns: '1fr 1fr', alignItems: 'start' }}>
        ...
        <Paper variant="outlined" sx={{ p: 1.25 }}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mb: 1 }}>
            <Button variant="outlined" onClick={handleGeneratePlan}>{t(K.page.coding.generatePlan)}</Button>
            <Button variant="contained" onClick={handleFreezePlan}>{t(K.page.coding.freezePlan)}</Button>
      ...
      <Box sx={{ px: 2, py: 1.5, display: 'flex', gap: 1.5, alignItems: 'center', flexWrap: 'wrap' }}>
        <Select size="small" value={askScope} onChange={(event) => setAskScope(event.target.value as any)} sx={{ minWidth: 120 }}>
          <MenuItem value="all">all</MenuItem>
          <MenuItem value="role">role</MenuItem>
          <MenuItem value="task">task</MenuItem>
          <MenuItem value="file">file</MenuItem>
        </Select>
        <TextField size="small" label={t(K.page.coding.askKeyLabel)} value={askKey} onChange={(event) => setAskKey(event.target.value)} sx={{ minWidth: 200 }} />
        <Button variant="outlined" onClick={handleAsk} disabled={!currentRunId}>{t(K.page.coding.askButton)}</Button>
      </Box>
