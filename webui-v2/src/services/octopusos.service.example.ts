...
import { octopusosServiceGen } from './octopusos.service.gen'
import type * as DTO from '@modules/octopusos'

/**
 * Extended OctopusOS service with custom functions
 *
...
export const octopusosService = {
  ...
  async listActiveProjects(
    ...
    return octopusosServiceGen.listProjects({
      ...params,
      status: 'active'
    })
  },
  ...
  async getProjectWithMetrics(id: string) {
    const project = await octopusosServiceGen.getProject(id)

    // Example: fetch additional data
    // const tasks = await octopusosServiceGen.listTasks({ project_id: id })
  ...
  async batchCreateProjects(
    ...
    return Promise.all(
      projects.map(project => octopusosServiceGen.createProject(project))
    )
  }
}
...
// Usage:
// export const octopusosService = withLogging(octopusosServiceGen)
...
export class AgentosService {
  ...
  private base = octopusosServiceGen
...
// Usage:
// export const octopusosService = new AgentosService()
