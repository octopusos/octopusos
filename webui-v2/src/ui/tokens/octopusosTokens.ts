/**
 * OctopusOS Semantic Tokens - Surface Layering System
 *
...
export const octopusosTokens = {
  ...
  /**
   * Shape Tokens (OctopusOS "device feel")
   */
  shape: {
    radius: {
  ...
  /**
   * Spacing Tokens (OctopusOS density)
   */
  spacing: {
 ...
 */
export const getOctopusOSTokens = (mode: 'light' | 'dark' | 'github' | 'google' | 'macos' | 'dracula' | 'nord' | 'monokai') => {
  // 自定义主题使用独立的配置
  if (mode === 'github' || mode === 'google' || mode === 'macos' || mode === 'dracula' || mode === 'nord' || mode === 'monokai') {
    const themeConfig = octopusosTokens[mode]
    return {
      bg: themeConfig.bg,
      ...
      shape: octopusosTokens.shape,
      spacing: octopusosTokens.spacing,
    }
  }

  // Light/Dark 主题使用原有逻辑
  return {
    bg: octopusosTokens[mode].bg,
    border: octopusosTokens[mode].border,
    text: octopusosTokens[mode].text,
    divider: octopusosTokens[mode].divider,
    status: octopusosTokens.status[mode],
    statusBg: octopusosTokens.statusBg[mode],
    elevation: octopusosTokens.elevation[mode],
    ...(mode === 'dark' && { surfaceHighlight: octopusosTokens.surfaceHighlight.dark }),
    brand: octopusosTokens.brand,
    shape: octopusosTokens.shape,
    spacing: octopusosTokens.spacing,
  }
}
...
// Type exports
export type OctopusOSTokens = ReturnType<typeof getOctopusOSTokens>
